...
def test_post_fetch_returns_202_and_get_returns_latest_execution(
    ...
    monkeypatch.setattr(
        fetch_execution, "_ensure_worker_locked", original_ensure_worker
    )
...
def test_post_fetch_uses_current_enqueue_signature(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_enqueue(
        run_dir_arg: Path, *, source: str | None = None, catalog_url: str | None = None
    ):
        captured["run_dir"] = run_dir_arg
        captured["source"] = source
        captured["catalog_url"] = catalog_url
        execution = fetch_execution._new_execution(
            run_dir_arg, source or "skroutz", catalog_url
        )
        fetch_execution.save_fetch_execution(execution)
        return execution

    monkeypatch.setattr(
        routes_price_monitoring, "enqueue_fetch_execution", fake_enqueue
    )
...
def test_execution_cli_fetch_completes_existing_execution(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    ...
    _write_execution_metadata(
        run_dir, "exec-1", status="running", queued_at=queued_at, started_at=queued_at
    )
    ...
    monkeypatch.setattr(
        run_price_monitoring_execution.fetch_execution,
        "run_price_monitoring_fetch",
        fake_fetch,
    )
    ...
    payload = (
        ...
        .get("/api/price-monitoring/runs/run-1/fetch/exec-1")
        .json()
    )
...
def test_execution_cli_fetch_failure_writes_failed_metadata(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    run_dir = _run_dir(tmp_path)
...
def test_successful_execution_writes_result_metadata_and_artifacts(
    tmp_path: Path, monkeypatch
) -> None:
    ...
    assert any(
        artifact["name"] == "fetch_result.json" for artifact in payload["artifacts"]
    )
    assert any(
        artifact["name"] == "fetch_execution.json" for artifact in payload["artifacts"]
    )


def test_nonzero_child_exit_marks_failed_when_child_writes_no_terminal_state(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    _write_run(_run_dir(tmp_path))
    ...
    assert (
        ...
        == "Execution failed. Artifacts may be partial or incomplete."
    )


def test_failed_execution_records_error_without_succeeded_status(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    _write_run(_run_dir(tmp_path))
...
def test_cancel_queued_execution_is_terminal_and_logged(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    _write_run(_run_dir(tmp_path))
    ...
    assert wait_for_worker_idle()
    assert (
        client.get("/api/price-monitoring/runs/run-1/fetch").json()["status"]
        == "cancelled"
    )
...
def test_cancel_running_execution_late_completion_does_not_persist_or_overwrite(
    ...
    cancelled = client.post(
        f"/api/price-monitoring/runs/run-1/fetch/{execution['execution_id']}/cancel"
    ).json()
    assert wait_for_worker_idle()
...
def test_cancel_running_execution_force_kills_after_timeout(
    tmp_path: Path, monkeypatch
) -> None:
    ...
    fetch_execution._ACTIVE_EXECUTIONS["exec-1"] = (
        fetch_execution._ActiveWorkerMetadata(
            worker_id="worker-1",
    ...
    monkeypatch.setattr(
        fetch_execution, "_send_graceful_terminate", lambda _process: None
    )
    monkeypatch.setattr(
        fetch_execution, "_force_kill_process_tree", lambda process: process.kill()
    )
    client = TestClient(create_app())

    killed = client.post("/api/price-monitoring/runs/run-1/fetch/exec-1/cancel").json()
    repeated = client.post(
        "/api/price-monitoring/runs/run-1/fetch/exec-1/cancel"
    ).json()
...
def test_killed_execution_does_not_block_refetch_and_increments_attempt(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    run_dir = _run_dir(tmp_path)
...
def test_different_runs_execute_concurrently_up_to_worker_cap(
    tmp_path: Path, monkeypatch
) -> None:
    ...
    assert (
        client.post(
            "/api/price-monitoring/runs/run-2/fetch", json={"source": "skroutz"}
        ).status_code
        == 202
    )
    assert _wait_for_status(client, "run-1", "running")
...
def test_worker_cap_queues_extra_runs_and_queue_position_clears_on_start(
    tmp_path: Path, monkeypatch
) -> None:
    ...
    assert (
        client.get("/api/price-monitoring/runs/run-2/fetch").json()["queue_position"]
        == 1
    )
...
def test_running_execution_exposes_worker_metadata_and_fresh_stale_state(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    _write_run(_run_dir(tmp_path))
...
def test_stale_detection_uses_configured_threshold_and_safe_fallback(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    run_dir = _run_dir(tmp_path)
    ...
    stale_payload = client.get(
        "/api/price-monitoring/runs/run-1/fetch/old-runner"
    ).json()
    ...
    fresh_payload = client.get(
        "/api/price-monitoring/runs/run-1/fetch/fresh-runner"
    ).json()
    assert fresh_payload["stale"] is True

    monkeypatch.setattr(
        fetch_execution, "_ACTIVE_EXECUTIONS", {"fresh-runner": object()}
    )
    owned_fresh_payload = client.get(
        "/api/price-monitoring/runs/run-1/fetch/fresh-runner"
    ).json()
    ...
    assert (
        client.get("/api/price-monitoring/runs/run-1/fetch/fresh-runner").json()[
            "stale_after_minutes"
        ]
        == 30
    )
    ...
    assert (
        client.get("/api/price-monitoring/runs/run-1/fetch/done").json()["stale"]
        is False
    )


def test_cancel_stale_running_execution_without_worker_marks_cancelled(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    run_dir = _run_dir(tmp_path)
    ...
    cancelled = client.post(
        "/api/price-monitoring/runs/run-1/fetch/stale-runner/cancel"
    ).json()
    lines = client.get(
        "/api/price-monitoring/runs/run-1/fetch/stale-runner/logs"
    ).json()["lines"]
...
def test_repeated_cancel_on_terminal_execution_is_idempotent_without_extra_logs(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    _write_run(_run_dir(tmp_path))
    ...
    first_lines = client.get(
        f"/api/price-monitoring/runs/run-1/fetch/{execution['execution_id']}/logs"
    ).json()["lines"]
    second = client.post(
        f"/api/price-monitoring/runs/run-1/fetch/{execution['execution_id']}/cancel"
    ).json()
...
def test_execution_listing_returns_sorted_items_with_stale_and_queue_position(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    run_dir = _run_dir(tmp_path)
    ...
    _write_execution_metadata(
        run_dir, "older", status="succeeded", queued_at=older, completed_at=older
    )
...
def test_execution_listing_empty_history_returns_empty_list(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    _write_run(_run_dir(tmp_path))
    ...
    payload = (
        TestClient(create_app())
        .get("/api/price-monitoring/runs/run-1/fetch/executions")
        .json()
    )
...
def test_legacy_fetch_result_statuses_map_to_execution_statuses(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    run_dir = _run_dir(tmp_path)
...
def test_old_execution_metadata_loads_with_new_artifact_fields(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    run_dir = _run_dir(tmp_path)
    ...
    now = datetime.now(timezone.utc).replace(microsecond=0).isoformat()
    _write_execution_metadata(
        run_dir, "old-json", status="succeeded", queued_at=now, completed_at=now
    )

    payload = (
        TestClient(create_app())
        .get("/api/price-monitoring/runs/run-1/fetch/old-json")
        .json()
    )
    legacy = fetch_execution.source_url_fetch_result_to_execution_payload(
        _legacy_fetch_result_object(run_dir)
    )
...
def test_specific_execution_and_logs_endpoints_return_lines(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    _write_run(_run_dir(tmp_path))
    ...
    specific = client.get(
        f"/api/price-monitoring/runs/run-1/fetch/{execution['execution_id']}"
    ).json()
    latest_logs = client.get("/api/price-monitoring/runs/run-1/fetch/logs").json()
    specific_logs = client.get(
        f"/api/price-monitoring/runs/run-1/fetch/{execution['execution_id']}/logs"
    ).json()
    ...
    assert any(
        "Started subprocess fetch execution" in line for line in specific_logs["lines"]
    )
...
def test_run_listing_latest_fetch_uses_execution_metadata(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    database_url = f"sqlite+pysqlite:///{tmp_path / 'ecommerce.db'}"
    ...
    list_payload = client.get("/api/price-monitoring/runs").json()["items"][0][
        "latest_fetch"
    ]
    detail_payload = client.get("/api/price-monitoring/runs/run-1").json()[
        "latest_fetch"
    ]
...
def test_late_child_save_cannot_overwrite_killed_execution(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    run_dir = _run_dir(tmp_path)
    ...
    now = datetime.now(timezone.utc).replace(microsecond=0).isoformat()
    _write_execution_metadata(
        run_dir, "exec-1", status="killed", queued_at=now, completed_at=now
    )
...
def test_dev_start_prints_urls_and_runs_backend_only(monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr(
        dev_start.uvicorn,
        "run",
        lambda app, **kwargs: run_args.update({"app": app, **kwargs}),
    )
    ...
    assert (
        "Price Monitoring DB status URL: http://127.0.0.2:8123/api/price-monitoring/db/status"
        in output
    )
...
def _use_fake_child(
    monkeypatch, tmp_path: Path, *, mode: str, sleep_seconds: float = 0.0
) -> None:
...
save(payload)
sys.exit(0)
""".replace("__MODE__", repr(mode)).replace(
            "__SLEEP_SECONDS__", repr(float(sleep_seconds))
        ),
        encoding="utf-8",
    )
...
def _wait_for_status(
    client: TestClient, run_id: str, status: str, timeout: float = 5.0
) -> bool:
    deadline = time.time() + timeout
    while time.time() < deadline:
        if (
            client.get(f"/api/price-monitoring/runs/{run_id}/fetch")
            .json()
            .get("status")
            == status
        ):
...
def _wait_for_active_process(execution_id: str, timeout: float = 5.0) -> bool:
    ...
    while time.time() < deadline:
        ...
        if (
            ...
            and metadata.process is not None
            and metadata.process.poll() is None
        ):
            return True
        time.sleep(0.02)
    return False
...
def _route_fetch_success(
    ...
    source_capture_result_path.write_text(
        json.dumps(source_capture_result.to_dict()), encoding="utf-8"
    )
