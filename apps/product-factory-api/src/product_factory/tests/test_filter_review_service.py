...
from product_factory.services.models import RunArtifacts, RunStatus, RunType
...
from product_factory.utils import ensure_directory, write_json
...
def review_workspace(tmp_path: Path, monkeypatch) -> dict[str, str]:
    ...
    monkeypatch.setattr(
        repo_paths, "FILTER_MAP_BASE_PATH", mappings_dir / "filter_map.base.json"
    )
    monkeypatch.setattr(
        repo_paths,
        "FILTER_MAP_MANUAL_OVERRIDES_PATH",
        mappings_dir / "filter_map.manual_overrides.json",
    )
    monkeypatch.setattr(
        repo_paths,
        "FILTER_MAP_MANUAL_OVERRIDES_BACKUP_DIR",
        mappings_dir / "backups" / "filter_overrides",
    )
    monkeypatch.setattr(repo_paths, "FILTER_MAP_PATH", mappings_dir / "filter_map.json")
    monkeypatch.setattr(
        repo_paths,
        "FILTER_MAP_SYNC_REPORT_PATH",
        mappings_dir / "filter_map.sync_report.json",
    )
        ...
        "filter_groups": [
            {
                ...
                "values": [
                    {
                        "value_id": stable_value_id(ram_group_id, "16 GB"),
                        "value": "16 GB",
                        "status": "active",
                    }
                ],
            },
            {
                ...
                "values": [
                    {
                        "value_id": stable_value_id(screen_group_id, "15.6"),
                        "value": "15.6",
                        "status": "active",
                    }
                ],
            },
            {
                ...
                "values": [
                    {
                        "value_id": stable_value_id(color_group_id, "Black"),
                        "value": "Black",
                        "status": "active",
                    }
                ],
            },
            {
                ...
                "values": [
                    {
                        "value_id": stable_value_id(old_group_id, "Legacy"),
                        "value": "Legacy",
                        "status": "deprecated",
                    }
                ],
...
def _write_prepared(
    model: str, *, taxonomy: TaxonomyResolution, specs: list[tuple[str, str]]
) -> None:
    model_root = repo_paths.model_root_path(model)
    scrape_dir = ensure_directory(model_root / "scrape")
    source = SourceProductData(
        key_specs=[SpecItem(label=label, value=value) for label, value in specs]
    )
    write_json(scrape_dir / f"{model}.source.json", source.to_dict())
    write_json(
        scrape_dir / f"{model}.normalized.json", {"taxonomy": taxonomy.to_dict()}
    )
...
def test_get_review_state_fails_when_prepared_artifacts_are_missing(
    review_workspace,
) -> None:
...
def test_get_review_state_loads_category_id_and_reports_missing_required(
    review_workspace,
) -> None:
    _write_prepared(
        MODEL,
        taxonomy=TaxonomyResolution(
            category_id=review_workspace["category_id"], taxonomy_path=CATEGORY_PATH
        ),
        specs=[("Memory", "16 GB")],
    )
    ...
    assert [group["group_name"] for group in payload["missing_required_groups"]] == [
        "Screen"
    ]
    assert "required_category_filter_missing:Screen" in payload["warnings"]


def test_get_review_state_falls_back_to_taxonomy_path_when_category_id_missing(
    review_workspace,
) -> None:
    _write_prepared(
        MODEL,
...
def test_get_review_state_includes_inactive_and_deprecated_diagnostics(
    review_workspace,
) -> None:
    _write_prepared(
        MODEL,
...
def test_put_saves_canonical_review_artifact_and_requires_reapproval(
    review_workspace,
) -> None:
    _write_prepared(
        MODEL,
    ...
    response = _client().put(
        ...
        json={
            "values": [
                {
                    "group_id": review_workspace["screen_group_id"],
                    "group_name": "Screen",
                    "value": "17",
                }
            ]
    ...
    assert (
        payload["values"][review_workspace["screen_group_id"]]["group_name"] == "Screen"
    )
...
def test_put_rejects_empty_group_names_and_values(review_workspace) -> None:
    ...
    assert (
        ...
        .put(
            f"/api/filter-review/{MODEL}",
            json={"values": [{"group_name": "", "value": "1"}]},
        )
        .status_code
        == 422
    )
    assert (
        ...
        .put(
            f"/api/filter-review/{MODEL}",
            json={"values": [{"group_name": "Screen", "value": ""}]},
        )
        .status_code
        == 422
    )
...
def test_put_new_value_updates_manual_overrides_and_regenerates_filter_map_without_approval(
    review_workspace,
) -> None:
    _write_prepared(
        MODEL,
    ...
    response = _client().put(
        ...
        json={
            "values": [
                {
                    "group_id": review_workspace["ram_group_id"],
                    "group_name": "Memory",
                    "value": "32 GB",
                }
            ]
    ...
    manual = json.loads(
        repo_paths.FILTER_MAP_MANUAL_OVERRIDES_PATH.read_text(encoding="utf-8")
    )
    value_id = stable_value_id(review_workspace["ram_group_id"], "32 GB")
    assert (
        manual["categories"][review_workspace["category_id"]]["groups"][
            review_workspace["ram_group_id"]
        ]["values"][value_id]["status"]
        == "active"
    )
    final_map = json.loads(repo_paths.FILTER_MAP_PATH.read_text(encoding="utf-8"))
    group = next(
        group
        for group in final_map["by_category_id"][review_workspace["category_id"]][
            "filter_groups"
        ]
        if group["group_id"] == review_workspace["ram_group_id"]
    )
    assert sum(1 for value in group["values"] if value["value"] == "32 GB") == 1


def test_put_new_group_defaults_required_active_and_regenerates_filter_map(
    review_workspace,
) -> None:
    _write_prepared(
        MODEL,
    ...
    response = _client().put(
        f"/api/filter-review/{MODEL}",
        json={"new_groups": [{"group_name": "Storage", "value": "512 GB"}]},
    )
    assert response.status_code == 200
    ...
    storage = manual["categories"][review_workspace["category_id"]]["groups"][
        storage_group_id
    ]
    ...
    final_map = json.loads(repo_paths.FILTER_MAP_PATH.read_text(encoding="utf-8"))
    assert any(
        group["group_id"] == storage_group_id
        for group in final_map["by_category_id"][review_workspace["category_id"]][
            "filter_groups"
        ]
    )
...
def test_approval_allows_missing_required_and_saves_review_value(
    review_workspace,
) -> None:
    _write_prepared(
        MODEL,
...
def test_put_group_update_changes_required_rule_and_regenerates_filter_map(
    review_workspace,
) -> None:
    _write_prepared(
        MODEL,
    ...
    screen = next(
        ...
        if group["group_id"] == review_workspace["screen_group_id"]
    )
    ...
    assert (
        manual["categories"][review_workspace["category_id"]]["groups"][
            review_workspace["screen_group_id"]
        ]["required"]
        is False
    )


def test_artifact_resolver_returns_category_filter_review_path_when_present(
    review_workspace,
) -> None:
    review_path = repo_paths.category_filter_review_path(MODEL)
    ensure_directory(review_path.parent)
    ...
    record = JobRecord(
        job_id="1", job_type=JobType.RENDER, status=JobStatus.SUCCEEDED, model=MODEL
    )
    artifacts = resolve_job_artifacts(record, repo_root=repo_paths.REPO_ROOT)
    assert any(
        artifact.name == "category_filter_review_path"
        and artifact.path == str(review_path)
        for artifact in artifacts
    )


def test_run_metadata_serializes_optional_category_filter_review_path(
    tmp_path: Path,
) -> None:
    ...
    metadata_path = metadata_module.maybe_write_run_metadata(
        ...
        artifacts=RunArtifacts(
            model_root=model_root, category_filter_review_path=review_path
        ),
