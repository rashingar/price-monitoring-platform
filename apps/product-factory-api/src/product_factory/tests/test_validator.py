...
def test_validator_detects_encoding_issue_and_missing_required_fields(
    tmp_path: Path,
) -> None:
    ...
    assert (
        "c1_control_character"
        in report["field_health"]["description"]["encoding_issues"]
    )
...
def test_validator_compares_against_baseline(tmp_path: Path) -> None:
    ...
    candidate = tmp_path / "candidate.csv"
    write_csv_row(
        {"model": "233541", "mpn": "GSGV80PYLL", "name": "LG GSGV80PYLL – Ψυγείο"},
        baseline,
        template,
    )
    write_csv_row(
    ...
    report = validate_candidate_csv(
        candidate, baseline_path=baseline, template_path=template
    )
