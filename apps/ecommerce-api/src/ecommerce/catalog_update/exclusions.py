...
def load_excluded_models(
    path: Path | None = None, *, repo_root_func: Callable[[], Path] = repo_root
) -> ExcludedModels:
    ...
    resolved_path = (
        ...
        if explicit_value
        else (repo_root_func() / DEFAULT_EXCLUDED_MODELS_RELATIVE_PATH)
    )
    if path is not None:
    ...
    if not resolved_path.exists():
        if explicit_path:
            raise CatalogUpdateError(
                f"Catalog exclusion file not found: {resolved_path}"
            )
        return ExcludedModels(
            path=resolved_path, found=False, explicit_path=False, models=frozenset()
        )
    ...
    try:
        ...
        raise CatalogUpdateError(
            f"Catalog exclusion file could not be read: {resolved_path}"
        ) from exc
    ...
    try:
        with source_cata_path.open(
            ...
            if "model" not in fieldnames:
                raise CatalogUpdateError(
                    "Catalog update exclusion filtering failed: sourceCata.csv is missing required model column."
                )
            rows = list(reader)
    except OSError as exc:
        raise CatalogUpdateError(
            f"Catalog update exclusion filtering failed: could not read {source_cata_path}"
        ) from exc
    ...
    try:
        with filtered_path.open("w", encoding="utf-8", newline="") as output_handle:
            writer = csv.DictWriter(
                output_handle, fieldnames=fieldnames, extrasaction="ignore"
            )
            writer.writeheader()
            writer.writerows(kept_rows)
    except OSError as exc:
        raise CatalogUpdateError(
            f"Catalog update exclusion filtering failed: could not write {filtered_path}"
        ) from exc
...
def excluded_models_from_rows(rows: list[list[str]], path: Path) -> set[str]:
    ...
    if max_columns == 1:
        return {
            str(row[0]).strip() for row in non_empty_rows if row and str(row[0]).strip()
        }

    raise CatalogUpdateError(
        f"Catalog exclusion file must contain a model header or a single model column: {path}"
    )
