...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Run an existing Ecommerce monitoring fetch now."
    )
    parser.add_argument(
        "--source",
        default=None,
        help="Stored source/vendor for the run. Source all is not allowed.",
    )
    parser.add_argument("--run-id", default="")
    parser.add_argument("--catalog-url", default=None, help=argparse.SUPPRESS)
    parser.add_argument(
        "--trigger-type", choices=("manual", "scheduled"), default="scheduled"
    )
    args = parser.parse_args(argv)

    if not args.run_id.strip():
        print(
            "--run-id is required until a safe default selection mechanism exists.",
            file=sys.stderr,
        )
        return 2

    try:
        run_dir = resolve_price_monitoring_run_dir(
            args.run_id, PRICE_MONITORING_RUNS_DIR
        )
    ...
    print(
        f"Starting price monitoring fetch: run_id={args.run_id} source={source_label}"
    )
    try:
        result = run_price_monitoring_fetch(
            run_dir, source=args.source, catalog_url=args.catalog_url
        )
        persistence = persist_fetch_result_if_configured(
            result, trigger_type=args.trigger_type
        )
    except (
        FileNotFoundError,
        ValueError,
