...
PRICE_MONITORING_DATABASE_REQUIRED_MESSAGE = (
    "PostgreSQL is required for Price Monitoring."
)
...
PRICE_MONITORING_DATABASE_REQUIRED_FOR = [
    "catalog",
    "price_monitoring",
    "alerts",
    "observations",
...
def collect_price_monitoring_database_readiness(
    ...
    status.update(
        {
            ...
            "ready_for_catalog": not _blocking_reasons(
                status, require_active_catalog=False
            ),
...
def collect_catalog_database_readiness(
    database_url: str | None = None,
) -> dict[str, Any]:
    status = dict(collect_database_status(database_url))
    ...
    status.update(
        {
            ...
            "price_monitoring_requires_database": True,
            "ready_for_price_monitoring": not _blocking_reasons(
                status, require_active_catalog=True
            ),
            "blocking_reasons": blocking_reasons,
            "non_db_workflows_available": True,
            "required_for": list(PRICE_MONITORING_DATABASE_REQUIRED_FOR),
...
def require_database_ready_for_price_monitoring() -> None:
    ...
    if not bool(readiness.get("ready_for_price_monitoring", False)):
        raise HTTPException(
            status_code=503,
            detail=price_monitoring_database_unavailable_detail(readiness),
        )
...
def catalog_database_unavailable_detail(
    ...
    return {
        ...
        "blocking_reasons": [
            str(item) for item in _list_value(status.get("blocking_reasons"))
        ],
        "setup_hints": [str(item) for item in _list_value(status.get("setup_hints"))],
    }


def price_monitoring_database_unavailable_detail(
    readiness: dict[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    return {
        ...
        "required_tables_present": bool(status.get("required_tables_present", False)),
        "alembic_up_to_date": bool(status.get("alembic_up_to_date", False)),
        "ready_for_price_monitoring": bool(
            status.get("ready_for_price_monitoring", False)
        ),
        "active_catalog_count": status.get("active_catalog_count"),
        "active_catalog_imported_at": status.get("active_catalog_imported_at"),
        "blocking_reasons": [
...
def _blocking_reasons(
    status: dict[str, Any], *, require_active_catalog: bool
) -> list[str]:
    reasons: list[str] = []
    ...
    if (
        ...
        and not reasons
        and int(status.get("active_catalog_count") or 0) <= 0
    ):
