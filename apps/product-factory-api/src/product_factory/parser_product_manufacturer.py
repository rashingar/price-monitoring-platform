...
LITERS_RE = re.compile(
    r"(\d+(?:[.,]\d+)?)\s*(?:lt|l\b|λίτρ(?:α|ο)?|λιτρ(?:α|ο)?)", re.IGNORECASE
)
...
class ManufacturerProductParser:
    def parse(
        ...
        *,
        ...
        if normalized_source == normalize_for_match("manufacturer_bosch"):
            return self._parse_bosch_product(
                html, url, source_name=source_name, fallback_used=fallback_used
            )
        ...
        return self._parse_tefal_product(
            html, url, source_name=source_name, fallback_used=fallback_used
        )
    ...
    def _parse_bosch_product(
        ...
        *,
        ...
        brand = self._normalize_bosch_brand(
            product.get("productBrand") or jsonld.get("brand") or "BOSCH"
        )
        ...
        energy_label = (
            product.get("energyLabel")
            if isinstance(product.get("energyLabel"), dict)
            else {}
        )
        energy_label_asset_url = resolve_eprel_energy_label_asset_url(
            ...
            eprel_registration_number=self._nested_text(
                product, ["energyLabel", "eprelRegistrationNumber"]
            ),
        )
        energy_label_pdf = self._nested_text(
            product, ["energyLabel", "pdf", "link", "value"]
        )
        product_sheet_asset_url = (
            self._nested_text(product, ["dataSheet", "url"]) or energy_label_pdf
        )
        presentation_source_html, presentation_source_text = (
            self._build_bosch_presentation_source(product)
        )
        ...
        source = SourceProductData(
            ...
            breadcrumbs=taxonomy_fields["breadcrumbs"],
            skroutz_family=(
                "built_in_appliance" if family_key == "built_in_oven" else family_key
            ),
            category_tag_text="",
            category_tag_href="",
            category_tag_slug="",
            ...
            taxonomy_escalation_reason=(
                "manufacturer_family_unresolved" if not family_key else ""
            ),
        ...
        diagnostics = {
            ...
            "brand": self._diagnostic(
                brand, provenance["brand"], 0.98 if brand else 0.0
            ),
            "product_code": self._diagnostic(
                mpn, provenance["product_code"], 0.98 if mpn else 0.0
            ),
            "price": self._diagnostic(None, provenance["price"], 0.0),
            "hero_summary": self._diagnostic(
                hero_summary, provenance["hero_summary"], 0.9 if hero_summary else 0.0
            ),
            "breadcrumbs": self._diagnostic(
                source.breadcrumbs,
                provenance["breadcrumbs"],
                0.9 if source.breadcrumbs else 0.0,
            ),
            "gallery_images": self._diagnostic(
                gallery_images,
                provenance["gallery_images"],
                0.94 if gallery_images else 0.0,
            ),
            "spec_sections": self._diagnostic(
                spec_sections,
                provenance["spec_sections"],
                0.94 if spec_sections else 0.0,
            ),
    ...
    def _parse_tefal_product(
        ...
        *,
        ...
        json_item = self._extract_json_item(
            soup, title=title, canonical_url=canonical_url
        )
        brand = (
            self._normalize_brand(str(json_item.get("item_brand", "")))
            or self._infer_brand_from_title(title)
            or "Tefal"
        )
        raw_product_code = normalize_whitespace(
            str(json_item.get("item_id", ""))
        ) or self._extract_product_code_from_scripts(html)
        ...
        presentation_source_text = normalize_whitespace(
            parsed_page.presentation_source_text
        )
        family_key = self._infer_tefal_family(
            title, canonical_url, hero_summary, presentation_source_text
        )
        ...
        all_spec_sections = self._merge_sections(
            feature_sections, manufacturer_sections
        )
        key_specs = self._build_key_specs(all_spec_sections)
        gallery_images = self._extract_gallery_images(
            soup, canonical_url, fallback_alt=title
        )
        warnings: list[str] = []
        if not family_key:
            warnings.append("manufacturer_family_unresolved")
        ...
        source = SourceProductData(
            ...
            manufacturer_source_text=normalize_whitespace(
                parsed_page.manufacturer_source_text
            ),
            presentation_source_html=presentation_source_html,
            presentation_source_text=presentation_source_text,
            raw_html_path="",
        ...
        product_code_strategy = (
            ...
            else "script:json-item" if json_item.get("item_id") else "regex:script"
        )
        provenance = {
            "name": "dom:h1",
            "brand": (
                "script:json-item" if json_item.get("item_brand") else "heuristic:title"
            ),
        ...
        diagnostics = {
            ...
            "product_code": self._diagnostic(
                product_code or mpn,
                provenance["product_code"],
                0.92 if product_code or mpn else 0.0,
            ),
            "price": self._diagnostic(
                price_text or price_value,
                provenance["price"],
                0.95 if price_text or price_value else 0.0,
            ),
            "hero_summary": self._diagnostic(
                hero_summary, provenance["hero_summary"], 0.94 if hero_summary else 0.0
            ),
            "breadcrumbs": self._diagnostic(
                source.breadcrumbs,
                provenance["breadcrumbs"],
            ...
            "gallery_images": self._diagnostic(
                gallery_images,
                provenance["gallery_images"],
                0.9 if gallery_images else 0.0,
            ),
            "spec_sections": self._diagnostic(
                all_spec_sections,
                provenance["spec_sections"],
                0.88 if all_spec_sections else 0.0,
            ),
        ...
        for script in soup.select("script[type='application/ld+json']"):
            ...
            for candidate in candidates:
                if (
                    ...
                    and normalize_for_match(candidate.get("@type", "")) == "product"
                ):
                    return candidate
        return {}

    def _build_bosch_title(
        self, product: dict[str, Any], jsonld: dict[str, Any], soup: BeautifulSoup
    ) -> str:
    ...
    def _infer_bosch_family(
        self, canonical_url: str, title: str, product: dict[str, Any]
    ) -> str:
        breadcrumbs = " ".join(
    ...
    def _build_bosch_taxonomy_fields(
        self, family_key: str
    ...
    def _build_bosch_spec_sections(self, product: dict[str, Any]) -> list[SpecSection]:
        ...
        for group in product.get("specifications") or []:
            ...
            if title and items:
                sections.append(
                    SpecSection(section=title, items=self._dedupe_items(items))
                )
        return sections

    def _build_bosch_key_specs(
        self, spec_sections: list[SpecSection], family_key: str
    ) -> list[SpecItem]:
        ...
        canonical_aliases = [
            (
                ...
                ["Ωφέλιμος όγκος θαλάμου - ΝΕΟ (2010/30/ΕΚ)", "Ωφέλιμος όγκος θαλάμου"],
            ),
        ...
        for canonical_label, aliases in canonical_aliases:
            value = next(
                (
                    specs_by_label[alias]
                    for alias in aliases
                    if specs_by_label.get(alias)
                ),
                "",
            )
            if value:
    ...
    def _bosch_spec_value(self, item: dict[str, Any]) -> str:
        ...
        if (
            unit
            and value
            and not normalize_for_match(value).endswith(normalize_for_match(unit))
        ):
            value = f"{value} {unit}"
    ...
    def _build_bosch_gallery_images(
        self, product: dict[str, Any], jsonld: dict[str, Any], title: str
    ) -> list[GalleryImage]:
        urls: list[str] = []
    ...
    def _build_bosch_presentation_source(
        ...
        for highlight in product.get("highlights") or []:
            ...
            blocks.append(
                f"<section><h3>{title}</h3><p>{body}</p>{image_html}</section>"
            )
    ...
    def _extract_json_item(
        self, soup: BeautifulSoup, *, title: str = "", canonical_url: str = ""
    ) -> dict[str, Any]:
        ...
        for script in soup.select(
            "script.json-item, script[type='application/ld+json']"
        ):
            raw = script.string or script.get_text(" ", strip=True)
            ...
            try:
                ...
                continue
            candidate = self._normalize_json_item_payload(
    ...
    def _normalize_json_item_payload(
        self, payload: Any, *, canonical_url: str
    ) -> dict[str, Any]:
        ...
        if not any(
            ...
            for key in ["item_name", "item_brand", "item_id", "price"]
        ):
            return {}
        return candidate
    ...
    def _extract_price(
        self, soup: BeautifulSoup, json_item: dict[str, Any]
    ...
    def _extract_gallery_images(
        self, soup: BeautifulSoup, base_url: str, fallback_alt: str
    ) -> list[GalleryImage]:
        ordered_urls = dedupe_urls_preserve_order(
            [
                image_url
                for slide in soup.select(
                    "clrz-slider-product .product-slider-main .swiper-slide"
                )
    ...
    def _extract_image_url(self, container: Tag, base_url: str) -> str:
        for node in container.select("img, source"):
            ...
            if not raw:
                raw = self._pick_first_srcset_candidate(
    ...
    def _infer_tefal_family(
        self, title: str, url: str, hero_summary: str, presentation_source_text: str
    ) -> str:
        haystack = normalize_for_match(
            " ".join([title, url, hero_summary, presentation_source_text])
        )
        if any(token in haystack for token in ["παγωτομηχαν", "pagotomichan"]):
            return "ice_cream_maker"
        if any(token in haystack for token in ["καφετιερ", "kafetier"]) and any(
            token in haystack for token in ["φιλτρ", "filtr"]
        ):
        ...
        *,
        ...
        if product_code:
            general_items.append(
                SpecItem(label="Κωδικός Προϊόντος", value=product_code)
            )
        ...
        if general_items:
            sections.append(
                SpecSection(section="Γενικά", items=self._dedupe_items(general_items))
            )
        ...
        if family_key == "ice_cream_maker":
            ...
            if programs:
                production_items.append(
                    SpecItem(label="Αριθμός Προγραμμάτων", value=programs)
                )
            if normalize_for_match(combined_text).find("vegan") >= 0:
                production_items.append(
                    SpecItem(label="Διατροφικές Επιλογές", value="Vegan")
                )
            if production_items:
                sections.append(
                    SpecSection(
                        section="Παραγωγή & Δυνατότητες",
                        items=self._dedupe_items(production_items),
                    )
                )
            ...
            if color:
                sections.append(
                    SpecSection(
                        ...
                        items=[SpecItem(label="Χρώμα", value=color)],
                    )
                )
        ...
        if family_key == "coffee_filter":
            ...
            if liters:
                overview_items.append(
                    SpecItem(label="Χωρητικότητα Δοχείου Νερού σε Λίτρα", value=liters)
                )
            cups = self._extract_cups(combined_text)
            if cups:
                overview_items.append(
                    SpecItem(label="Χωρητικότητα σε Φλιτζάνια", value=cups)
                )
            if overview_items:
                sections.append(
                    SpecSection(
                        section="Επισκόπηση Προϊόντος",
                        items=self._dedupe_items(overview_items),
                    )
                )
            color = self._extract_color(title, url, combined_text)
            if color:
        ...
        if family_key == "kettle":
            ...
            if liters:
                overview_items.append(
                    SpecItem(label="Χωρητικότητα σε Λίτρα", value=liters)
                )
            power = self._extract_power(combined_text)
            if power:
    ...
    def _merge_sections(
        self, primary: list[SpecSection], secondary: list[SpecSection]
    ) -> list[SpecSection]:
        merged = [
            SpecSection(section=section.section, items=list(section.items))
        ...
        return [
            SpecSection(
                section=section.section, items=self._dedupe_items(section.items)
            )
    ...
    def _extract_color(self, title: str, url: str, combined_text: str) -> str:
        haystacks = [
            ...
            normalize_for_match(combined_text),
        ]
        color_map = [
            ("kafe", "Καφέ"),
            ("καφε", "Καφέ"),
    ...
    def _dedupe_items(self, items: list[SpecItem]) -> list[SpecItem]:
        ...
        for item in items:
            key = (
                ...
                normalize_for_match(item.value or ""),
            )
    ...
    def _diagnostic(
        self, value: Any, selected_strategy: str, confidence: float
    ) -> FieldDiagnostic:
