...
def _run_from_catalog(args: argparse.Namespace) -> int:
    if not is_database_configured():
        print("from-catalog requires ECOMMERCE_DATABASE_URL.", file=sys.stderr)
        return 1
    active_only = _bool_arg(args.active_only)
    with session_scope() as session:
...
def _apply_review(args: argparse.Namespace) -> int:
    ...
    if apply and not is_database_configured():
        print("apply-review --apply requires ECOMMERCE_DATABASE_URL.", file=sys.stderr)
        return 1
...
def _export_candidates(args: argparse.Namespace) -> int:
    if not is_database_configured():
        print("export-candidates requires ECOMMERCE_DATABASE_URL.", file=sys.stderr)
        return 1
    with session_scope() as session:
...
def _import_candidates(args: argparse.Namespace) -> int:
    if not is_database_configured():
        print("import-candidates requires ECOMMERCE_DATABASE_URL.", file=sys.stderr)
        return 1
    apply = bool(args.apply and not args.dry_run)
    with session_scope() as session:
