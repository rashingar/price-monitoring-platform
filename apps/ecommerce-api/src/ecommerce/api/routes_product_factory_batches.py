...
router = APIRouter(
    prefix="/api/product-factory-batches", tags=["product-factory-batches"]
)
...
def list_product_factory_batches(
    ...
    try:
        with session_scope() as session:
            return {
                "items": [
                    repository.batch_to_dict(batch)
...
def resolve_product_factory_batch(
    ...
    try:
        with session_scope() as session:
            ...
            if batch is None:
                raise HTTPException(status_code=404, detail="Batch not found.")
            start = prepare_batch_resolution(
                session,
                batch=batch,
            ...
            payload = {
                **repository.batch_to_dict(batch),
                "rows": [repository.row_to_dict(row) for row in rows],
            }
        if should_start:
            _schedule_batch_resolution(
                background_tasks, batch_id=batch_id, source_names=source_names
            )
        return payload
        ...
        raise HTTPException(
            status_code=500,
            detail={"code": "source_resolution_config_error", "message": str(exc)},
        ) from exc
    except DatabaseNotConfiguredError as exc:
        raise _database_unavailable(exc) from exc
    except SQLAlchemyError as exc:
...
@router.post(
    "/{batch_id}/rows/{row_id}/select-source",
    response_model=ProductFactoryBatchRowResponse,
)
def select_product_factory_batch_row_source(
    batch_id: int, row_id: int, request: SelectSourceRequest
) -> dict[str, Any]:
    try:
        with session_scope() as session:
            batch = repository.get_batch(session, batch_id)
...
@router.post(
    "/{batch_id}/rows/{row_id}/skip", response_model=ProductFactoryBatchRowResponse
)
...
@router.post(
    "/{batch_id}/enqueue-selected", response_model=ProductFactoryBatchEnqueueResponse
)
def enqueue_selected_product_factory_batch_rows(batch_id: int) -> dict[str, Any]:
    try:
        with session_scope() as session:
            ...
            if batch is None:
                raise HTTPException(status_code=404, detail="Batch not found.")
            result = enqueue_batch_selected(
                session, batch=batch, product_factory_client=_product_factory_client()
            )
            return {
...
@router.post(
    "/{batch_id}/rows/{row_id}/enqueue", response_model=ProductFactoryBatchRowResponse
)
def enqueue_product_factory_batch_row(batch_id: int, row_id: int) -> dict[str, Any]:
    try:
        with session_scope() as session:
            ...
            if row is None:
                raise HTTPException(status_code=404, detail="Batch row not found.")
            row = enqueue_batch_row(
                session,
                batch=batch,
                row=row,
                product_factory_client=_product_factory_client(),
            )
            return repository.row_to_dict(row)
    except ProductFactoryBatchError as exc:
        raise HTTPException(
...
@router.post(
    "/{batch_id}/refresh-job-statuses",
    response_model=ProductFactoryBatchJobStatusRefreshResponse,
)
def refresh_product_factory_batch_job_statuses(batch_id: int) -> dict[str, Any]:
    try:
        with session_scope() as session:
            ...
            if batch is None:
                raise HTTPException(status_code=404, detail="Batch not found.")
            result = refresh_batch_job_statuses(
                session, batch=batch, product_factory_client=_product_factory_client()
            )
            return {
                "batch_id": result.batch_id,
...
async def _multipart_csv_upload(request: Request) -> tuple[bytes, str | None]:
    ...
    if not message.is_multipart():
        raise HTTPException(
            status_code=400,
            detail={
                "code": "invalid_multipart_csv",
                "message": "Upload body is not valid multipart data.",
            },
        )
    ...
    raise HTTPException(
        ...
        detail={
            ...
            "message": "Multipart upload must include a file field.",
        },
    )
...
def _schedule_batch_resolution(
    ...
    *,
    ...
    background_tasks.add_task(
        run_batch_resolution_background, batch_id=batch_id, source_names=source_names
    )
