...
- From `scraper/`, use `..\.venv\Scripts\python.exe -m ...`.
...
1. Run `..\.venv\Scripts\python.exe -m pipeline.workflow prepare ...` from `scraper/`.
2. Read:
...
4. Run `..\.venv\Scripts\python.exe -m pipeline.workflow render --model {model}` from `scraper/`.
