...
def test_worker_picks_queued_job_and_runs_registered_catalog_handler(
    ...
    monkeypatch.setattr(
        catalog_update_service, "run_catalog_update_durable_job", fake_catalog_update
    )
    with session_scope(database_url) as session:
        create_queued_job(
            session, job_type=CATALOG_UPDATE_JOB_TYPE, payload={}, job_id="job-1"
        )

    result = run_worker_iteration(
        registry=build_default_registry(), database_url=database_url
    )
...
def test_worker_honors_job_type_filter(tmp_path: Path) -> None:
    ...
    registry.register(
        CATALOG_UPDATE_JOB_TYPE,
        lambda job_id, _payload: calls.append(job_id) or {"ok": job_id},
    )
    registry.register(
        "other_job", lambda job_id, _payload: calls.append(job_id) or {"ok": job_id}
    )
    ...
    with session_scope(database_url) as session:
        ...
        create_queued_job(
            session, job_type=CATALOG_UPDATE_JOB_TYPE, payload={}, job_id="job-catalog"
        )

    result = run_worker_iteration(
        registry=registry, job_type=CATALOG_UPDATE_JOB_TYPE, database_url=database_url
    )
...
def test_worker_honors_pre_start_cancellation(tmp_path: Path) -> None:
    ...
    calls: list[str] = []
    registry.register(
        CATALOG_UPDATE_JOB_TYPE, lambda job_id, _payload: calls.append(job_id)
    )
...
def test_worker_marks_catalog_update_stale_after_job_type_threshold(
    tmp_path: Path,
) -> None:
    database_url = _database_url(tmp_path)
    _create_schema(database_url)
    ...
    with session_scope(database_url) as session:
        create_queued_job(
            ...
            payload={},
            job_id="job-catalog-stale",
        )
        mark_running(
            session, "job-catalog-stale", started_at=now - timedelta(minutes=300)
        )
...
def test_worker_does_not_fail_catalog_update_with_recent_progress(
    tmp_path: Path,
) -> None:
    database_url = _database_url(tmp_path)
    _create_schema(database_url)
    ...
    with session_scope(database_url) as session:
        create_queued_job(
            session, job_type=CATALOG_UPDATE_JOB_TYPE, payload={}, job_id="job-active"
        )
        mark_running(session, "job-active", started_at=now - timedelta(minutes=90))
        record_progress(
            session,
            "job-active",
            step="wait_for_download",
...
def test_worker_dry_run_does_not_execute_handler_or_mutate_jobs(tmp_path: Path) -> None:
    ...
    registry.register(
        CATALOG_UPDATE_JOB_TYPE,
        lambda job_id, _payload: calls.append(job_id) or {"ok": True},
    )
    ...
    result = run_worker_iteration(
        registry=registry, database_url=database_url, dry_run=True
    )
