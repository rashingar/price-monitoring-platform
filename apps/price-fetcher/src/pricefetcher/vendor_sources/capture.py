...
from pricefetcher.db.models import CatalogProductRow, Product, ProductSource, SourceUrl, Vendor, VendorSourceCaptureRun
from pricefetcher.price_monitoring.source_url_coverage import compute_source_url_coverage
...
from pricefetcher.source_capture.runner import capture_source_url
from pricefetcher.source_urls import infer_source_name
    ...
    *,
    ...
    if not is_database_configured():
        result = _empty_result(
            ...
            warnings=["Source URL capture skipped because PRICEFETCHER_DATABASE_URL is not configured."],
        )
        if write_result:
    ...
    with session_scope() as session:
        vendor_run_id = _make_vendor_capture_run_id()
        observation_batch_id = vendor_run_id
        row = _create_vendor_source_capture_run_row(
            session,
        ...
        _update_vendor_source_capture_run(row, result)
    if write_result:
        _write_result(result_path, result)
    return result
...
def capture_selected_source_urls(
    ...
    *,
    ...
    from pricefetcher.db.source_convergence import sync_source_url_to_product_source
    from pricefetcher.db.source_url_repository import (
        list_active_source_urls_for_catalog_products,
        source_url_to_dict,
    )
    from pricefetcher.source_capture.scheduled import capture_due_product_sources
...
def run_vendor_source_capture(
    ...
    *,
    ...
    run_id = _make_vendor_capture_run_id()
    ...
    row = VendorSourceCaptureRun(
        ...
        catalog_source=normalized_catalog_source,
        warnings_json=[],
        filters_json=_json_safe_value(filters),
        artifact_refs_json=[],
...
def _eligible_product_sources_from_active_source_urls(
    ...
    *,
    ...
    from pricefetcher.db.source_convergence import sync_product_source_to_source_url, sync_source_url_to_product_source
    from pricefetcher.db.source_url_repository import source_url_to_dict
...
def _selected_product_source_context(session: Session, product_source_ids: list[int]) -> tuple[int, list[dict[str, Any]]]:
    if not product_source_ids:
        return 0, []
    from pricefetcher.db.source_convergence import sync_product_source_to_source_url
    from pricefetcher.db.source_url_repository import source_url_to_dict
...
def _coverage_item(catalog_product_id: int):
    from pricefetcher.price_monitoring.selection import SelectedPriceMonitoringProduct
...
def _create_vendor_source_capture_run_row(
    session: Session,
    *,
    run_id: str,
    observation_batch_id: str,
...
def _update_vendor_source_capture_run(row: VendorSourceCaptureRun, result: SourceUrlCaptureRunResult) -> None:
    now = _now()
...
def _make_vendor_capture_run_id() -> str:
