...
from .skroutz_taxonomy import (
    classify_skroutz_taxonomy,
...
SUMMARY_PAIR_RE = re.compile(
    r"([^:]{2,80}?):\s*(.+?)(?=(?:\s+[A-Za-zΑ-ΩΆ-Ώα-ωά-ώ][^:]{1,40}:)|$)"
)
...
SKROUTZ_FAMILIES: dict[str, dict[str, Any]] = {
    ...
    "coffee_filter": {
        ...
        "sections": [
            (
                ...
                [
                    "Χωρητικότητα σε Φλυτζάνια",
                    "Υλικό Κανάτας",
                    "Ισχύς σε Watts",
                    "Χωρητικότητα Δοχείου Νερού σε Λίτρα",
        ...
        "sections": [
            (
                ...
                [
                    ...
                    "Βάση 360 Μοιρών",
                    "Φίλτρο Νερού",
                    "Άνοιγμα του Καπακιού με το Πάτημα ενός Κουμπιού",
                    "Καλυμμένη Αντίσταση",
                    "Υλικό Κατασκευής",
                    "Αποθήκευση Καλωδίου",
                ],
    ...
    "tabletop_hob": {
        ...
        "sections": [
            ("Χαρακτηριστικά Μοντέλου", ["Κατασκευαστής", "Μοντέλο"]),
            ("Γενικά Χαρακτηριστικά", ["Εστία"]),
    ...
    "fryer": {
        "category_labels": {"Φριτέζες"},
        "category_href_tokens": {"fritezes", "friteza", "fryers"},
        "title_tokens": {"friteza", "fritezes", "fry 3", "fryer", "air fryer"},
        "breadcrumbs": ["Αρχική", "ΟΙΚΙΑΚΟΣ ΕΞΟΠΛΙΣΜΟΣ", "Μικροί Μάγειρες", "Φριτέζες"],
        "sections": [],
        "spec_mode": "raw",
        "taxonomy_mode": "family",
    },
    ...
    "microwave": {
        ...
        "category_href_tokens": {
            "fournoi mikrokymatwn",
            "fournoi mikrokymaton",
            "mikrokymatwn",
            "mikrokymaton",
        },
    ...
    "laundry": {
        ...
        "category_href_tokens": {
            "plyntiria stegnotiria",
            "plynthria stegnwthria",
            "plyntiria",
            "plynthria",
            "stegnotiria",
        },
    ...
    "built_in_appliance": {
        ...
        "category_href_tokens": {
            "entoixizomenes",
            "entoixizomenes syskeyes",
            "entoichizomenes syskeves",
            "esties kouzinas",
        },
    ...
    "home_appliance_accessory": {
        ...
        "category_href_tokens": {
            "axesouar oikiakon syskeyon",
            "aksesouar oikiakon syskevon",
        },
    ...
    "stand_fan": {
        "category_labels": set(),
        "category_href_tokens": {"orthostatis", "dapedou", "stand fan", "pedestal fan"},
        "title_tokens": {
            "ορθοστατης",
            "δαπεδου",
            "orthostatis",
        ...
        "breadcrumbs": ["Αρχική", "ΚΛΙΜΑΤΙΣΜΟΣ ΘΕΡΜΑΝΣΗ", "Ανεμιστήρες", "Ορθοστάτης"],
        "sections": [],
        "spec_mode": "raw",
        "taxonomy_mode": "family",
    },
    ...
    "ceiling_fan": {
        ...
        "title_tokens": {"οροφης", "orofis", "orofhs", "ceiling fan"},
        "breadcrumbs": ["Αρχική", "ΚΛΙΜΑΤΙΣΜΟΣ ΘΕΡΜΑΝΣΗ", "Ανεμιστήρες", "Οροφής"],
        "sections": [],
        "spec_mode": "raw",
        "taxonomy_mode": "family",
    },
    ...
    "stick_vacuum": {
        ...
        "title_tokens": {
            ...
            "stick handheld",
            "2 se 1 stick",
            "epanafortizomeni skoupa",
        },
        "breadcrumbs": ["Αρχική", "ΟΙΚΙΑΚΟΣ ΕΞΟΠΛΙΣΜΟΣ", "Σκούπισμα", "Σκούπες Stick"],
        "sections": [],
        "spec_mode": "raw",
        "taxonomy_mode": "family",
    },
...
class SkroutzProductParser:
    def parse(self, html: str, url: str, fallback_used: bool = False) -> ParsedProduct:
        ...
        if self._is_interstitial(soup):
            ...
            return ParsedProduct(
                ...
                warnings=["skroutz_interstitial_detected"],
                critical_missing=[
                    "name",
                    "breadcrumbs",
                    "gallery_images",
        ...
        diagnostics["name"] = self._make_diagnostic(
            title, title_source, 0.99 if title else 0.0, title_trace
        )

        category_tag_text, category_tag_href, category_source, category_trace = (
            self._extract_category_tag(soup, jsonld, canonical_url)
        )
        provenance["category_tag"] = category_source
        diagnostics["category_tag"] = self._make_diagnostic(
            category_tag_text or category_tag_href,
            category_source,
            0.95 if category_tag_text or category_tag_href else 0.0,
            category_trace,
        )

        family = self._resolve_family(
            category_tag_text, category_tag_href, title, canonical_url
        )
        family_config = SKROUTZ_FAMILIES.get(family, {})

        merchant_titles = self._extract_merchant_titles(soup)
        brand, brand_source, brand_trace = self._extract_brand(
            soup, jsonld, title, merchant_titles
        )
        provenance["brand"] = brand_source
        diagnostics["brand"] = self._make_diagnostic(
            brand, brand_source, 0.96 if brand else 0.0, brand_trace
        )

        product_code, code_source, code_trace = self._extract_page_sku(
            soup, jsonld, canonical_url
        )
        provenance["product_code"] = code_source
        diagnostics["product_code"] = self._make_diagnostic(
            product_code, code_source, 0.95 if product_code else 0.0, code_trace
        )

        summary_html, hero_summary, summary_source, summary_trace, summary_pairs = (
            self._extract_summary_block(soup)
        )
        provenance["hero_summary"] = summary_source
        diagnostics["hero_summary"] = self._make_diagnostic(
            hero_summary, summary_source, 0.88 if hero_summary else 0.0, summary_trace
        )
        ...
        mpn, mpn_source, mpn_trace = self._extract_mpn(
            soup, jsonld, title, brand, product_code, merchant_titles
        )
        provenance["mpn"] = mpn_source
        diagnostics["mpn"] = self._make_diagnostic(
            mpn, mpn_source, 0.92 if mpn else 0.0, mpn_trace
        )

        price_text, price_value, price_source, price_trace = self._extract_price(
            soup, jsonld
        )
        provenance["price"] = price_source
        diagnostics["price"] = self._make_diagnostic(
            price_text or price_value,
            price_source,
            0.95 if price_text or price_value else 0.0,
            price_trace,
        )
        ...
        if taxonomy_hint and taxonomy_hint.ambiguous:
            warnings.append(
                f"skroutz_taxonomy_ambiguous:{taxonomy_hint.escalation_reason}"
            )
        ...
        diagnostics["breadcrumbs"] = self._make_diagnostic(
            breadcrumbs,
            provenance["breadcrumbs"],
            0.95 if breadcrumbs else 0.0,
            category_trace,
        )
        ...
        if (
            family
            and family_config.get("taxonomy_mode") != "helper"
            and len(breadcrumbs) >= 3
        ):
        ...
        diagnostics["spec_sections"] = self._make_diagnostic(
            canonical_sections,
            spec_source,
            0.92 if canonical_sections else 0.0,
            spec_trace,
        )
        key_specs = self._build_key_specs(
            family, canonical_sections, raw_sections, raw_pairs, summary_pairs
        )
        provenance["key_specs"] = "canonical_from_specs" if key_specs else "missing"
        diagnostics["key_specs"] = self._make_diagnostic(
            key_specs, provenance["key_specs"], 0.9 if key_specs else 0.0, []
        )
        energy_label_asset_url = resolve_eprel_energy_label_asset_url(
            family_key=family,
            breadcrumbs=breadcrumbs,
            taxonomy_source_category=(
                taxonomy_hint.source_category
                if taxonomy_hint and not taxonomy_hint.ambiguous
                else family_source_category
            ),
            title=title,
        ...
        gallery_images, gallery_source, gallery_trace = self._extract_gallery_images(
            soup, jsonld, canonical_url, title
        )
        provenance["gallery_images"] = gallery_source
        diagnostics["gallery_images"] = self._make_diagnostic(
            gallery_images,
            gallery_source,
            0.95 if gallery_images else 0.0,
            gallery_trace,
        )
        ...
        source = SourceProductData(
            ...
            taxonomy_match_type=(
                taxonomy_hint.match_type
                if taxonomy_hint and not taxonomy_hint.ambiguous
                else family_match_type
            ),
            taxonomy_rule_id=(
                taxonomy_hint.matched_rule_id
                if taxonomy_hint
                else (f"family:{family}" if family else "")
            ),
            taxonomy_ambiguity=bool(
                taxonomy_hint.ambiguous if taxonomy_hint else False
            ),
    ...
    def _parse_product_jsonld(self, soup: BeautifulSoup) -> dict[str, Any]:
        node = soup.select_one("#product-schema") or soup.select_one(
    ...
    def _extract_title(
        self, soup: BeautifulSoup, jsonld: dict[str, Any]
    ) -> tuple[str, str, list[SelectorTraceEntry]]:
        trace: list[SelectorTraceEntry] = []
        node = soup.select_one("div.sku-title h1.page-title")
        trace.append(
            self._trace(
                "dom", "div.sku-title h1.page-title", [node] if node else [], node
            )
        )
        if node:
    ...
    def _extract_category_tag(
        ...
        trace.append(
            self._trace(
                "dom", "div.sku-title a.category-tag", [node] if node else [], node
            )
        )
    ...
    def _resolve_family(
        ...
        for family, config in SKROUTZ_FAMILIES.items():
            labels = {
                ...
                for label in config.get("category_labels", set())
            }
            href_tokens = {
                normalize_for_match(token)
                for token in config.get("category_href_tokens", set())
            }
            if labels and category_norm in labels:
                return family
            if href_tokens and any(
                token and token in slug_norm for token in href_tokens
            ):
                return family
        for family, config in SKROUTZ_FAMILIES.items():
            title_tokens = {
            ...
            if family == "soundbar" and "soundbar" in title_norm:
                return family
            if (
                family == "coffee_filter"
                and "καφετιερ" in title_norm
                and "φιλτρ" in title_norm
            ):
                return family
    ...
    def _extract_brand(
        ...
        for selector in ["a.brand-page-link img", "a.brand-page-link span"]:
            ...
            value = (
                normalize_whitespace(
                    node.get("alt") if node.name == "img" else safe_text(node)
                )
                .replace(" στο Skroutz", "")
                .strip()
            )
        ...
        return (
            (fallback, "heuristic:title_brand", trace)
            if fallback
            else ("", "missing", trace)
        )
    ...
    def _extract_page_sku(
        self, soup: BeautifulSoup, jsonld: dict[str, Any], url: str
    ) -> tuple[str, str, list[SelectorTraceEntry]]:
        trace: list[SelectorTraceEntry] = []
        node = soup.select_one("h1 small.sku-code")
        trace.append(
            self._trace("dom", "h1 small.sku-code", [node] if node else [], node)
        )
        if node:
        ...
        return (
            (match.group(1), "url:path_sku", trace) if match else ("", "missing", trace)
        )
    ...
    def _extract_mpn(
        ...
        trace.append(
            self._trace(
                "dom",
                "#specs dt:-soup-contains('Κωδικός Προϊόντος') + dd",
                [node] if node else [],
                node,
            )
        )
        ...
        return (
            (best, "heuristic:model_token", trace) if best else ("", "missing", trace)
        )

    def _extract_price(
        self, soup: BeautifulSoup, jsonld: dict[str, Any]
    ) -> tuple[str, float | None, str, list[SelectorTraceEntry]]:
        trace: list[SelectorTraceEntry] = []
        node = soup.select_one(".prices .final-price")
        trace.append(
            self._trace("dom", ".prices .final-price", [node] if node else [], node)
        )
        if node:
    ...
    ) -> tuple[str, str, str, list[SelectorTraceEntry], list[tuple[str, str, str]]]:
        trace: list[SelectorTraceEntry] = []
        selectors = [
            ".sku-description",
            "#description .simple-description",
            "div.summary .description.long",
            "div.summary .description.bullets",
        ]
    ...
    def _extract_raw_specs(
        self, soup: BeautifulSoup
    ) -> tuple[
        list[SpecSection], list[tuple[str, str, str]], str, list[SelectorTraceEntry]
    ]:
        trace: list[SelectorTraceEntry] = []
        containers = soup.select(
            "#specs .spec-groups, #combined-specs-content .spec-groups"
        )
        trace.append(
            self._trace(
                "dom",
        ...
        for container in containers:
            for detail in container.find_all(
                "div", class_="spec-details", recursive=False
            ):
                section_title = (
                    safe_text(detail.find("h3", recursive=False)) or "Γενικά"
                )
                items: list[SpecItem] = []
    ...
    def _extract_gallery_images(
        self, soup: BeautifulSoup, jsonld: dict[str, Any], url: str, title: str
    ) -> tuple[list[GalleryImage], str, list[SelectorTraceEntry]]:
        trace: list[SelectorTraceEntry] = []
        ...
        if urls:
            ...
            return (
                [
                    GalleryImage(url=item, alt=title, position=index)
        ...
        nodes = soup.select(
            "img[data-modal-data], img[data-sku-page--gallery--swipe-gallery-target='image']"
        )
        trace.append(
            self._trace(
                "dom", "img[data-modal-data]", nodes, nodes[0] if nodes else None
            )
        )
        for node in nodes:
            src = (
    ...
    def _build_bsh_energy_label_icon_url(self, brand: str, energy_class: str) -> str:
        if normalize_for_match(brand) not in {
            normalize_for_match("Bosch"),
            normalize_for_match("Neff"),
        }:
            return ""
    ...
    def _extract_merchant_titles(self, soup: BeautifulSoup) -> list[str]:
        titles = [
            normalize_whitespace(node.get("title") or safe_text(node))
            for node in soup.select(
                "#prices .product-name[title], .selected-product-cards .product-name[title]"
            )
        ]
    ...
    def _best_model_token(self, texts: list[str], brand: str) -> str:
        ...
        for text in texts:
            for raw in normalize_whitespace(text).split():
                ...
                if (
                    not token
                    or normalize_for_match(token) == brand_norm
                    or not MODEL_TOKEN_RE.match(upper)
                ):
                    continue
                score = 10 + sum(
                    bool(re.search(pattern, upper)) for pattern in [r"[A-Z]", r"\d"]
                )
                score += len(upper)
                if score > best_score:
                    best = upper
    ...
    def _build_canonical_sections(
        ...
        full_text = normalize_whitespace(
            " ".join([title, hero_summary, *merchant_titles, *lookup.values()])
        )
        ...
        if family == "coffee_filter":
            ...
            section_values = self._kettle_values(
                lookup, title, full_text, merchant_titles
            )
        else:
            section_values = self._tabletop_hob_values(
                lookup, brand, mpn, title, full_text
            )

        out: list[SpecSection] = []
        for section_title, labels in SKROUTZ_FAMILIES[family]["sections"]:
            items = [
                SpecItem(label=label, value=section_values.get(label, "-"))
                for label in labels
            ]
    ...
    def _coffee_values(
        self, lookup: dict[str, str], full_text: str, merchant_titles: list[str]
    ) -> dict[str, str]:
        normalized_text = normalize_for_match(full_text)
        dimensions = self._extract_dimensions_text(full_text, expected_parts=3)
        return {
            "Χωρητικότητα σε Φλυτζάνια": self._extract_cups(full_text)
            or self._normalize_cups(
                lookup.get(normalize_for_match("Χωρητικότητα Φλυτζάνια"))
            )
            or "-",
            "Υλικό Κανάτας": self._clean_placeholder(
                lookup.get(normalize_for_match("Υλικό Κανάτας"))
                or lookup.get(normalize_for_match("Κανάτα"))
            ),
            "Ισχύς σε Watts": self._number_only(
                lookup.get(normalize_for_match("Ισχύς"))
            ),
            "Χωρητικότητα Δοχείου Νερού σε Λίτρα": self._extract_capacity_from_titles(
                merchant_titles
            )
            or self._number_only(lookup.get(normalize_for_match("Χωρητικότητα lt"))),
            "Ένδειξη Στάθμης Νερού": self._bool_from_text(
                normalized_text, ["σταθμη", "νερου"]
            ),
            "Αποσπώμενο Δοχείο Νερού": self._boolean_or_placeholder(
                lookup.get(normalize_for_match("Αποσπώμενο Δοχείο Νερού"))
            ),
            "Θερμαινόμενη Βάση": self._boolean_or_placeholder(
                lookup.get(normalize_for_match("Θερμαινόμενη Βάση"))
            ),
            "Ενδεικτική Λυχνία Λειτουργίας": self._boolean_or_placeholder(
                lookup.get(normalize_for_match("Ενδεικτική Λυχνία Λειτουργίας"))
            ),
            "Ηχητικό Σήμα Ειδοποίησης": self._boolean_or_placeholder(
                lookup.get(normalize_for_match("Ηχητικό Σήμα Ειδοποίησης"))
            ),
            "Αποθήκευση Καλωδίου": self._boolean_or_placeholder(
                lookup.get(normalize_for_match("Αποθήκευση Καλωδίου"))
            ),
            "Διακοπή Σταξίματος": self._bool_from_text(normalized_text, ["σταξ"]),
            "Λειτουργία Aroma": self._boolean_or_placeholder(
                lookup.get(normalize_for_match("Λειτουργία Aroma"))
            ),
            "Αυτόματη Διακοπή Λειτουργίας": self._bool_from_text(
                normalized_text, ["αυτοματ"]
            ),
            "Χρονοδιακόπτης Λειτουργίας": self._boolean_or_placeholder(
                lookup.get(normalize_for_match("Χρονοδιακόπτης Λειτουργίας"))
            ),
    ...
    def _kettle_values(
        ...
        normalized_text = normalize_for_match(full_text)
        dimensions = self._extract_dimensions_text(
            " ".join(merchant_titles + [full_text]), expected_parts=3
        )
        return {
            "Χωρητικότητα σε Λίτρα": self._number_only(
                lookup.get(normalize_for_match("Χωρητικότητα"))
            ),
            "Ισχύς σε Watts": self._number_only(
                lookup.get(normalize_for_match("Ισχύς"))
            ),
            "Αποσπώμενη Βάση": self._boolean_or_placeholder(
                lookup.get(normalize_for_match("Αποσπώμενη Βάση"))
            ),
            "Βάση 360 Μοιρών": self._bool_from_text(normalized_text, ["360"]),
            "Φίλτρο Νερού": self._clean_placeholder(
                lookup.get(normalize_for_match("Φίλτρο Νερού"))
                or lookup.get(normalize_for_match("Είδος Φίλτρου"))
            ),
            "Άνοιγμα του Καπακιού με το Πάτημα ενός Κουμπιού": self._bool_from_text(
                normalized_text, ["καπακ", "κουμπ"]
            ),
            "Καλυμμένη Αντίσταση": self._boolean_or_placeholder(
                lookup.get(normalize_for_match("Καλυμμένη Αντίσταση"))
            ),
            "Υλικό Κατασκευής": self._clean_placeholder(
                lookup.get(normalize_for_match("Υλικό Κανάτας"))
            ),
            ...
            "Επιλογές Θερμοκρασίας Βρασμού": self._boolean_or_placeholder(
                lookup.get(normalize_for_match("Επιλογή Θερμοκρασίας"))
            ),
            "Λειτουργία Διατήρησης Θερμοκρασίας": self._boolean_or_placeholder(
                lookup.get(normalize_for_match("Διατήρηση Θερμοκρασίας"))
            ),
            "Αυτόματος Τερματισμός Λειτουργίας": self._bool_from_text(
                normalized_text, ["αυτοματ"]
            ),
            "Προστασία από Βρασμό Χωρίς Νερό": self._boolean_or_placeholder(
                lookup.get(normalize_for_match("Προστασία από Βρασμό Χωρίς Νερό"))
            ),
            "Χρώμα": self._clean_placeholder(lookup.get(normalize_for_match("Χρώμα"))),
            "Βάρος Συσκευής σε Κιλά": self._extract_weight(full_text),
            "Διαστάσεις Συσκευής σε Εκατοστά. (Υ χ Π χ Β": dimensions or "-",
            "Εγγύηση Κατασκευαστή": self._extract_warranty(full_text),
            "_title_color_suffix": self._title_color_suffix(
                title, lookup.get(normalize_for_match("Χρώμα"))
            ),
        }

    def _tabletop_hob_values(
        self, lookup: dict[str, str], brand: str, mpn: str, title: str, full_text: str
    ) -> dict[str, str]:
        normalized_title = normalize_for_match(title)
        width, depth = self._extract_dimensions_2(full_text)
        surface = self._clean_placeholder(
            lookup.get(normalize_for_match("Τύπος Εστίας"))
        )
        ...
        return {
            ...
            "_Είδος Διακόπτη": self._clean_placeholder(
                lookup.get(normalize_for_match("Είδος Διακόπτη"))
            ),
        }
    ...
    def _build_key_specs(
        ...
        if not family or SKROUTZ_FAMILIES.get(family, {}).get("spec_mode") == "raw":
            return self._build_generic_key_specs(
                raw_sections or sections, summary_pairs
            )
        lookup = {
            item.label: item.value or "-"
            for section in sections
            for item in section.items
        }
        ...
        items = [
            SpecItem(label=label, value=lookup[label])
    ...
    def _build_generic_key_specs(
        self, sections: list[SpecSection], summary_pairs: list[tuple[str, str, str]]
    ) -> list[SpecItem]:
        items: list[SpecItem] = []
        seen: set[str] = set()
        for section in sections:
    ...
    def _soundbar_sections(
        self, raw_pairs: list[tuple[str, str, str]], merchant_titles: list[str]
    ) -> list[SpecSection]:
        grouped: dict[str, list[SpecItem]] = {}
        for section_title, label, value in raw_pairs:
            normalized_title = (
                normalize_whitespace(section_title) or "Τεχνικά Χαρακτηριστικά"
            )
            grouped.setdefault(normalized_title, []).append(
                SpecItem(label=label, value=value)
            )
        derived_power = self._extract_soundbar_power(" ".join(merchant_titles))
        if derived_power:
            target_section = next(iter(grouped), "Χαρακτηριστικά")
            grouped.setdefault(target_section, []).append(
                SpecItem(label="Ισχύς", value=derived_power)
            )
        return [
            SpecSection(section=title, items=items) for title, items in grouped.items()
        ]

    def _extract_soundbar_power(self, text: str) -> str:
        matches = re.findall(
            r"(\d+(?:[.,]\d+)?)\s*W\b", text or "", flags=re.IGNORECASE
        )
        if not matches:
            return ""
    ...
    def _number_only(self, value: str | None) -> str:
        ...
        return (
            match.group(0).replace(".", ",") if match else normalize_whitespace(value)
        )
    ...
    def _collect_critical_missing(
        self, source: SourceProductData, supported_page: bool
    ) -> list[str]:
        critical: list[str] = []
        if not source.name:
    ...
    def _trace(
        self, strategy: str, selector: str, nodes: list[Tag], chosen: Tag | None
    ) -> SelectorTraceEntry:
        return SelectorTraceEntry(
            ...
            chosen_preview=safe_text(chosen)[:120] if chosen else "",
            note="",
        )
