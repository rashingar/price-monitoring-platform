...
class BestPriceProvider(ProductProvider):
    definition = ProviderDefinition(
        provider_id="bestprice",
        ...
        display_name="BestPrice",
        description="BestPrice vendor-site provider adapter.",
    )
    ...
    def fetch_snapshot(self, identity: ProviderInputIdentity) -> ProviderSnapshot:
        ...
        if not url:
            raise ProviderError.build(
                ...
                message="BestPrice provider requires a URL identity",
            )
    ...
    def _snapshot_from_fixture(self, identity: ProviderInputIdentity, url: str, fixture_path: Path) -> ProviderSnapshot:
        if not fixture_path.exists():
            raise ProviderError.build(
                ...
                stage=ProviderStage.FETCH,
                message=f"BestPrice fixture does not exist: {fixture_path}",
        ...
        try:
            ...
            raise ProviderError.build(
                ...
                code=ProviderErrorCode.FETCH_FAILED,
                stage=ProviderStage.FETCH,
                message=f"Failed to read BestPrice fixture: {fixture_path}",
                details={"url": url, "fixture_path": str(fixture_path)},
                cause=exc,
            ) from exc
