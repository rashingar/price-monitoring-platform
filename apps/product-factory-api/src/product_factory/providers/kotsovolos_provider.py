...
class KotsovolosProvider(ProductProvider):
    definition = ProviderDefinition(
        provider_id="kotsovolos",
        ...
        display_name="Kotsovolos",
        description="Kotsovolos vendor-site provider adapter.",
    )
    ...
    def __init__(
        ...
        *,
        ...
        parser: KotsovolosProductParser | None = None,
    ) -> None:
        ...
        self._parser = parser or KotsovolosProductParser()
    ...
    def fetch_snapshot(self, identity: ProviderInputIdentity) -> ProviderSnapshot:
        ...
        if not url:
            raise ProviderError.build(
                ...
                message="Kotsovolos provider requires a URL identity",
            )
    ...
    def _snapshot_from_fixture(
        ...
        if not fixture_path.exists():
            raise ProviderError.build(
                ...
                stage=ProviderStage.FETCH,
                message=f"Kotsovolos fixture does not exist: {fixture_path}",
        ...
        try:
            ...
            raise ProviderError.build(
                ...
                code=ProviderErrorCode.FETCH_FAILED,
                stage=ProviderStage.FETCH,
                message=f"Failed to read Kotsovolos fixture: {fixture_path}",
                details={"url": url, "fixture_path": str(fixture_path)},
                cause=exc,
            ) from exc
