...
from pricefetcher.bridge import DEFAULT_STOCK_CSV_PATH, run_bridge_from_balance_csv
...
def run_bridge(request: BridgeRunRequest) -> dict:
    ...
    run_id = _make_run_id()
    output_dir = Path(request.output_dir) if request.output_dir else Path("output") / "bridge" / "runs" / run_id
