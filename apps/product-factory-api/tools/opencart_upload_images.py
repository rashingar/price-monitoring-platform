...
try:
    ...
    ModuleNotFoundError
):  # pragma: no cover - unit tests may import helpers without runtime deps installed
    requests = None
...
def natural_key(name: str) -> list[Any]:
    return [
        int(tok) if tok.isdigit() else tok.lower() for tok in re.split(r"(\d+)", name)
    ]
    ...
    *,
    ...
    return {
        ...
        "bescos": {
            ...
            "remote_files": [
                f"catalog/{besco_remote_dir}/{p.name}" for p in besco_files
            ],
...
def login(
    session: requests.Session, admin_index: str, username: str, password: str
) -> str:
...
def permission_probe(
    session: requests.Session, admin_index: str, user_token: str
) -> dict[str, Any]:
    ...
    return {
        ...
        "can_modify": (not _is_permission_denied_error(error_text))
        and _is_missing_directory_error(error_text),
    }
...
def ensure_remote_nested_dir(
    session: requests.Session, admin_index: str, user_token: str, remote_dir: str
) -> None:
    parent = ""
...
def chunked_file_paths(
    file_paths: list[str], batch_size: int | None
) -> list[list[str]]:
...
def upload_files(
    ...
    *,
    ...
    if len(batches) == 1:
        return _upload_file_batch(
            session, admin_index, user_token, remote_dir, batches[0]
        )

    batch_results: list[dict[str, Any]] = []
    for batch in batches:
        batch_results.append(
            _upload_file_batch(session, admin_index, user_token, remote_dir, batch)
        )

    success_messages = [
        str(result.get("success", "")).strip()
        for result in batch_results
        if str(result.get("success", "")).strip()
    ]
    return {
        "success": (
            success_messages[-1] if success_messages else "Batch upload completed."
        ),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Upload repo-native product images to OpenCart filemanager."
    )
...
def main() -> int:
    ...
    session = requests.Session()
    user_token = login(
        session, admin_index, resolved_config["username"], resolved_config["password"]
    )
    ...
    if probe["permission_denied"]:
        raise UploadError(
            "Logged in, but the admin user lacks modify permission on common/filemanager."
        )
    ...
    ensure_remote_nested_dir(
        session, admin_index, user_token, plan["gallery"]["remote_dir"]
    )
    uploads["gallery"] = upload_files(
        session,
        admin_index,
        user_token,
        plan["gallery"]["remote_dir"],
        plan["gallery"]["local_files"],
    )

    if plan["bescos"]["count"] > 0:
        ensure_remote_nested_dir(
            session, admin_index, user_token, plan["bescos"]["remote_dir"]
        )
