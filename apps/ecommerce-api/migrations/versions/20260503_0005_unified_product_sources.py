...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "supports_direct_product_url",
            sa.Boolean(),
            nullable=False,
            server_default=sa.true(),
        ),
        sa.Column(
            "supports_search", sa.Boolean(), nullable=False, server_default=sa.false()
        ),
        sa.Column(
            "supports_xhr_capture",
            sa.Boolean(),
            nullable=False,
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.String(),
            nullable=False,
            server_default="direct_product_url",
        ),
        sa.Column("active", sa.Boolean(), nullable=False, server_default=sa.true()),
        ...
        sa.UniqueConstraint(
            "product_id",
            "canonical_url_hash",
            name="uq_product_sources_product_canonical_url_hash",
        ),
    )
    ...
    op.create_index(
        "ix_product_sources_canonical_url_hash",
        "product_sources",
        ["canonical_url_hash"],
    )
    op.create_index("ix_product_sources_active", "product_sources", ["active"])
    op.create_index(
        "ix_product_sources_last_seen_at", "product_sources", ["last_seen_at"]
    )
    op.create_index(
        "ix_product_sources_last_success_at", "product_sources", ["last_success_at"]
    )
    ...
    op.create_index(
        "ix_source_capture_snapshots_product_id",
        "source_capture_snapshots",
        ["product_id"],
    )
    op.create_index(
        "ix_source_capture_snapshots_vendor_id",
        "source_capture_snapshots",
        ["vendor_id"],
    )
    op.create_index(
        "ix_source_capture_snapshots_content_hash",
        "source_capture_snapshots",
        ["content_hash"],
    )
    ...
    with op.batch_alter_table("price_observations") as batch:
        batch.alter_column(
            "monitoring_run_id", existing_type=sa.Integer(), nullable=True
        )
        ...
        batch.create_foreign_key(
            "fk_price_observations_product_source_id",
            "product_sources",
            ["product_source_id"],
            ["id"],
            ondelete="SET NULL",
        ...
        batch.create_foreign_key(
            "fk_price_observations_vendor_id",
            "vendors",
            ["vendor_id"],
            ["id"],
            ondelete="SET NULL",
        ...
        batch.create_index(
            "ix_price_observations_product_source_id_observed_at",
            ["product_source_id", "observed_at"],
        )
        batch.create_index(
            "ix_price_observations_vendor_id_observed_at", ["vendor_id", "observed_at"]
        )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["aggregator_vendor_id"], ["vendors.id"], ondelete="SET NULL"
        ),
        sa.ForeignKeyConstraint(
            ["source_capture_snapshot_id"],
        ...
        sa.ForeignKeyConstraint(
            ["seller_vendor_id"], ["vendors.id"], ondelete="SET NULL"
        ),
    )
    op.create_index(
        "ix_offer_observations_product_id_observed_at",
        "offer_observations",
        ["product_id", "observed_at"],
    )
    op.create_index(
        "ix_offer_observations_product_source_id_observed_at",
        "offer_observations",
        ["product_source_id", "observed_at"],
    )
    op.create_index(
        "ix_offer_observations_aggregator_vendor_id",
        "offer_observations",
        ["aggregator_vendor_id"],
    )
    op.create_index(
        "ix_offer_observations_seller_vendor_id",
        "offer_observations",
        ["seller_vendor_id"],
    )
...
def downgrade() -> None:
    op.drop_index(
        "ix_offer_observations_seller_vendor_id", table_name="offer_observations"
    )
    op.drop_index(
        "ix_offer_observations_aggregator_vendor_id", table_name="offer_observations"
    )
    op.drop_index(
        "ix_offer_observations_product_source_id_observed_at",
    ...
    op.drop_index(
        "ix_offer_observations_product_id_observed_at", table_name="offer_observations"
    )
    ...
    with op.batch_alter_table("price_observations") as batch:
        ...
        batch.drop_constraint(
            "fk_price_observations_source_capture_snapshot_id", type_="foreignkey"
        )
        batch.drop_constraint("fk_price_observations_vendor_id", type_="foreignkey")
        batch.drop_constraint(
            "fk_price_observations_product_source_id", type_="foreignkey"
        )
        ...
        batch.alter_column(
            "monitoring_run_id", existing_type=sa.Integer(), nullable=False
        )
    op.drop_index(
        "ix_source_capture_snapshots_content_hash",
        table_name="source_capture_snapshots",
    )
    op.drop_index(
        "ix_source_capture_snapshots_vendor_id", table_name="source_capture_snapshots"
    )
    op.drop_index(
        "ix_source_capture_snapshots_product_id", table_name="source_capture_snapshots"
    )
    op.drop_index(
        "ix_source_capture_snapshots_product_source_id_captured_at",
        table_name="source_capture_snapshots",
...
def _seed_vendors() -> None:
    ...
    op.bulk_insert(
        sa.table("vendors", *[sa.column(key) for key in rows[0].keys()]), rows
    )
