...
class ProductFactorySourceResolver:
    ...
    def resolve(
        ...
        *,
        ...
        scoped = (
            self.source_scoped_queries
            if source_scoped_queries is None
            else source_scoped_queries
        )
        queries = (
            build_source_scoped_queries(product, self.config)
            if scoped
            else build_queries(product)
        )
        raw_candidates: list[SourceResolutionCandidate] = []
        seen_urls: set[str] = set()
        for query in queries:
            for item in self.fetcher.search(
                ...
                confidence = score_candidate(
                    product=product, source=source, item=item, url=url
                )
        ...
        candidates = tuple(
            sorted(
                raw_candidates,
                key=lambda item: (-item.confidence, item.result_rank or 9999, item.url),
            )
        )
        selected = (
            candidates[0]
            if candidates and candidates[0].confidence >= self.config.minimum_confidence
            else None
        )
