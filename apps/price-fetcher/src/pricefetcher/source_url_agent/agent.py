...
from pricefetcher.db.config import DatabaseNotConfiguredError
from pricefetcher.db.models import SourceUrl
from pricefetcher.source_url_agent.artifacts import (
    SourceUrlAgentArtifactPaths,
    build_summary_payload,
    write_run_artifacts,
)
from pricefetcher.source_url_agent.browser import SourceUrlBrowserSession
from pricefetcher.source_url_agent.candidates import (
...
from pricefetcher.source_url_agent.persistence import (
...
from pricefetcher.source_url_agent.scoring import score_candidate
from pricefetcher.source_url_agent.search import SourceSearchResult, discover_source_evidence, generate_search_queries
from pricefetcher.source_url_agent.sources import SourceDefinition, SourceRegistry, load_source_registry
...
def run_source_url_agent(
    *,
    ...
    if options.missing_only and session is None:
        warnings.append("missing_only was requested but the database is unavailable; missing coverage cannot be checked.")
    ...
    if resolver is not None:
        ...
        try:
            ...
            warnings.append(f"Browser unavailable; all tasks marked error: {str(exc).strip() or exc.__class__.__name__}")
    ...
    if apply_source_urls:
        ...
        warnings.append("apply_high_confidence requested but database is not configured; no source_urls were written.")
