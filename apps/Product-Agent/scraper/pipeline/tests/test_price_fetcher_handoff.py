...
from pipeline.price_fetcher_handoff import build_price_fetcher_source_handoff, price_fetcher_handoff_path
...
def test_build_price_fetcher_source_handoff_has_stable_contract_shape(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr("pipeline.price_fetcher_handoff.utcnow_iso", lambda: "2026-05-05T10:20:30+00:00")
...
def test_prepare_stage_writes_price_fetcher_handoff_after_success(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr("pipeline.price_fetcher_handoff.utcnow_iso", lambda: "2026-05-05T10:20:30+00:00")
    cli = _build_cli(tmp_path)
    ...
    handoff_path = price_fetcher_handoff_path(scrape_dir)
    assert handoff_path == price_fetcher_handoff_path(scrape_dir)
