...
def test_validate_templates_passes_on_valid_minimal_template_set(
    tmp_path: Path,
) -> None:
    ...
    results, duplicate_messages = validate_all_templates(
        template_root=template_root, schema_path=DEFAULT_SCHEMA_PATH
    )

    assert duplicate_messages == []
    assert len(results) == 1
    assert results[0][1] == []
    assert "Summary: checked=1 valid=1 invalid=0 issues=0" in render_report(
        results, duplicate_messages
    )
...
def test_validate_templates_fails_on_duplicate_template_ids(tmp_path: Path) -> None:
    ...
    _write_json(
        template_root / "demo_b.json",
        {**_valid_template("demo_b"), "id": "shared_electronet_labels_v1"},
    )
    _write_json(
        template_root / "shared.json",
        {**_valid_template("shared"), "id": "shared_electronet_labels_v1"},
    )
    ...
    assert any(
        "duplicate logical template id" in message for message in duplicate_messages
    )
    assert any(results)


def test_validate_templates_fails_on_duplicate_labels_in_one_section(
    tmp_path: Path,
) -> None:
    template_root = tmp_path / "templates"
    template_root.mkdir()
    ...
    results, _ = validate_all_templates(
        template_root=template_root, schema_path=DEFAULT_SCHEMA_PATH
    )
