...
class ProductFactoryBatch(Base):
    ...
    manually_selected_count: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, server_default="0"
    )
    needs_review_count: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, server_default="0"
    )
    no_usable_source_count: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, server_default="0"
    )
    resolution_failed_count: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, server_default="0"
    )
...
class ProductFactoryBatchRow(Base):
    ...
    batch_id: Mapped[int] = mapped_column(
        ForeignKey("product_factory_batches.id", ondelete="CASCADE"), nullable=False
    )
    ...
    candidate_urls_json: Mapped[list[dict[str, Any]] | None] = mapped_column(
        JSON_DOCUMENT, nullable=True
    )
    error_code: Mapped[str | None] = mapped_column(String, nullable=True)
    error_message: Mapped[str | None] = mapped_column(Text, nullable=True)
    selection_metadata_json: Mapped[dict[str, Any] | None] = mapped_column(
        JSON_DOCUMENT, nullable=True
    )
    product_factory_job_id: Mapped[str | None] = mapped_column(String, nullable=True)
    product_factory_job_status: Mapped[str | None] = mapped_column(
        String, nullable=True
    )
    product_factory_job_message: Mapped[str | None] = mapped_column(Text, nullable=True)
    product_factory_error_code: Mapped[str | None] = mapped_column(
        String, nullable=True
    )
    product_factory_error_message: Mapped[str | None] = mapped_column(
        Text, nullable=True
    )
    enqueued_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    job_status_refreshed_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False
