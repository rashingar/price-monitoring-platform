...
def test_skroutz_fridge_freezer_uses_requested_name_schema() -> None:
    ...
    assert fields["seo_keyword"] == "bosch-kgn36nlea-psygeiokatapsyktis-total-no-frost-305lt-e"
...
def test_tv_name_uses_size_resolution_panel_and_concrete_platform() -> None:
        ...
        spec_sections=[
            ...
            SpecSection(
                ...
                items=[
                    ...
                    SpecItem(label="Λειτουργίες Smart", value="Google Assistant,Google TV"),
                ],
            ),
        ],
    )
    ...
    fields = build_deterministic_product_fields(source, taxonomy, "142659", derive_seo_keyword)
...
def test_tv_name_uses_smart_tv_only_when_no_specific_platform_exists() -> None:
    ...
    fields = build_deterministic_product_fields(source, taxonomy, "141435", derive_seo_keyword)
...
def test_electronet_built_in_oven_capacity_alias_keeps_liter_unit() -> None:
    ...
    fields = build_deterministic_product_fields(source, taxonomy, "234070", derive_seo_keyword)

    assert fields["name"] == "Bosch HBA514BS3 – Φούρνος ηλεκτρικός 71Lt A+ Inox"
    assert fields["meta_title"] == "Bosch HBA514BS3 Φούρνος ηλεκτρικός 71Lt | eTranoulis"
...
def test_fridge_cabinet_subcategory_owns_category_phrase() -> None:
    ...
    fields = build_deterministic_product_fields(source, taxonomy, "231025", derive_seo_keyword)

    assert fields["name"] == "Bosch KFN96AXEA – Ψυγείο Ντουλάπα No Frost 605Lt E"
    assert fields["meta_title"] == "Bosch KFN96AXEA Ψυγείο Ντουλάπα No Frost 605Lt | eTranoulis"
...
def test_color_rule_recovers_title_color_and_omits_irrelevant_type_value() -> None:
    ...
    fields = build_deterministic_product_fields(source, taxonomy, "340864", derive_seo_keyword)
...
def test_kettle_rule_uses_single_capacity_differentiator() -> None:
    ...
    fields = build_deterministic_product_fields(source, taxonomy, "344347", derive_seo_keyword)
...
def test_egg_boiler_uses_specific_kettle_category_phrase() -> None:
    source = SourceProductData(
        ...
        key_specs=[SpecItem(label="Ισχύς", value="400 W"), SpecItem(label="Χρώμα", value="Μαύρο")],
    )
    taxonomy = TaxonomyResolution(
        parent_category="ΟΙΚΙΑΚΟΣ ΕΞΟΠΛΙΣΜΟΣ",
        leaf_category="Συσκευές Κουζίνας",
    ...
    fields = build_deterministic_product_fields(source, taxonomy, "340144", derive_seo_keyword)

    assert fields["name"] == "Philips HD9137/90 – Βραστήρας Αυγών 400W Μαύρο"
    assert fields["meta_title"] == "Philips HD9137/90 Βραστήρας Αυγών 400W Μαύρο | eTranoulis"
    assert fields["seo_keyword"] == "philips-hd9137-90-vrastiras-aygon-6-theseon-400w-mayros"
...
def test_deterministic_fields_never_use_product_code_as_mpn_and_extract_unicode_model() -> None:
    source = SourceProductData(
        source_name="electronet",
        brand="Tefal",
        product_code="339576",
    ...
    fields = build_deterministic_product_fields(source, taxonomy, "339576", derive_seo_keyword)
...
def test_deterministic_fields_extract_spaced_mpn_and_do_not_inject_specs_into_complete_title() -> None:
    source = SourceProductData(
        ...
        key_specs=[
            SpecItem(label="Τεχνολογία Πλατώ Εστιών", value="Αυτόνομο κεραμικό ηλεκτρικό"),
            SpecItem(label="Αριθμός Ζωνών", value="5 ηλεκτρικές (4+1 διπλού δακτυλίου)"),
        ],
        spec_sections=[
            SpecSection(section="Γενικά", items=[SpecItem(label="Πλάτος Διάστασης Εντοιχισμού σε Εκατοστά", value="56,00")]),
        ],
    )
    taxonomy = TaxonomyResolution(
        parent_category="ΟΙΚΙΑΚΕΣ ΣΥΣΚΕΥΕΣ",
    ...
    fields = build_deterministic_product_fields(source, taxonomy, "226826", derive_seo_keyword)
...
def test_apply_name_rule_dedupes_tv_resolution_and_prefers_concrete_platform_from_analysi_othonis() -> None:
    ...
    assert compose_name("TCL", "115C7K", category_phrase, differentiators) == 'TCL 115C7K – Τηλεόραση Mini LED 115" 8K UHD Google TV'
