...
`web` is a React, Vite, and TypeScript dashboard for operators working with the
local Product Factory and ecommerce-api services.
...
- A dashboard for checking Product Factory and commerce service health.
- Prepare, render, publish, and job detail pages for Product Factory workflows.
- A staged Product Factory workflow page for authoring, filter review, settings, render, and
  publish operations.
...
- Product Factory API running at `http://127.0.0.1:8000`.
- ecommerce-api running at `http://127.0.0.1:8001`.
...
- Browser requests to `/api` are proxied to the Product Factory API.
