...
def _normalize_section(
    raw_section: dict[str, Any] | None, *, source_index: int
    ...
    body_raw = (
        raw_section.get("body_text")
        or raw_section.get("paragraph")
        or raw_section.get("body_html")
        or raw_section.get("body")
        or ""
    )
...
def _build_metrics(
    *, title: str, body_text: str, image_url: str, media_html: str
    ...
    unique_word_ratio = (
        (len(set(normalized_tokens)) / len(normalized_tokens))
...
def _classify_section(
    *,
    ...
    if (
        metrics.word_count < MISSING_MIN_WORDS
        or metrics.alphabetic_char_count < MISSING_MIN_ALPHA_CHARS
    ):
        return MISSING_QUALITY, (
            REASON_MISSING_IMAGE_ONLY
            if image_url and metrics.alphabetic_char_count == 0
            else REASON_MISSING_EMPTY_AFTER_CLEAN
        )
    ...
    if not title:
        return WEAK_QUALITY, REASON_WEAK_MISSING_TITLE
    if (
        metrics.word_count >= USABLE_MIN_WORDS
        and metrics.alphabetic_char_count >= USABLE_MIN_ALPHA_CHARS
    ):
