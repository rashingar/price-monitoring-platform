...
class MonitoringRun(Base):
    ...
    last_was_refetch: Mapped[bool] = mapped_column(
...
class CatalogSnapshot(Base):
    ...
    raw_catalog_row: Mapped[dict[str, Any] | None] = mapped_column(
    ...
    monitoring_run: Mapped[MonitoringRun] = relationship(
        back_populates="catalog_snapshots"
    )
...
class PriceObservation(Base):
    ...
    product_source_id: Mapped[int | None] = mapped_column(
        ForeignKey("product_sources.id", ondelete="SET NULL"), nullable=True
    )
    ...
    raw_observation: Mapped[dict[str, Any] | None] = mapped_column(
        JSON_DOCUMENT, nullable=True
    )
    matched_by: Mapped[str | None] = mapped_column(String, nullable=True)
    match_status: Mapped[str] = mapped_column(
        String, nullable=False, default="unmatched", server_default="unmatched"
    )
...
class PriceObservationListing(Base):
    ...
    __table_args__ = (
        Index(
            "ix_price_observation_listings_price_observation_id", "price_observation_id"
        ),
        Index("ix_price_observation_listings_run_id", "run_id"),
        Index(
            "ix_price_observation_listings_observation_batch_id", "observation_batch_id"
        ),
    ...
    monitoring_run_id: Mapped[int | None] = mapped_column(
        ForeignKey("monitoring_runs.id", ondelete="CASCADE"), nullable=True
    )
    run_id: Mapped[str] = mapped_column(String, nullable=False)
    observation_batch_id: Mapped[str | None] = mapped_column(String, nullable=True)
    product_id: Mapped[int | None] = mapped_column(
    ...
    delivery_text: Mapped[str | None] = mapped_column(Text, nullable=True)
    product_url: Mapped[str | None] = mapped_column(Text, nullable=True)
    raw_listing: Mapped[dict[str, Any] | None] = mapped_column(
        JSON_DOCUMENT, nullable=True
    )
    observed_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False
...
class OfferObservation(Base):
    ...
    __table_args__ = (
        Index(
            "ix_offer_observations_product_id_observed_at", "product_id", "observed_at"
        ),
    ...
    aggregator_vendor_id: Mapped[int | None] = mapped_column(
        ForeignKey("vendors.id", ondelete="SET NULL"), nullable=True
    )
    source_capture_snapshot_id: Mapped[int | None] = mapped_column(
        ForeignKey("source_capture_snapshots.id", ondelete="SET NULL"),
    ...
    seller_name: Mapped[str | None] = mapped_column(String, nullable=True)
    seller_url: Mapped[str | None] = mapped_column(Text, nullable=True)
    seller_vendor_id: Mapped[int | None] = mapped_column(
        ForeignKey("vendors.id", ondelete="SET NULL"), nullable=True
    )
    price: Mapped[Decimal | None] = mapped_column(Numeric(12, 2), nullable=True)
    original_price: Mapped[Decimal | None] = mapped_column(
