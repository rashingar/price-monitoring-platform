...
PRICE_MONITORING_RUNS_DIR = Path("output") / "price_monitoring" / "runs"
...
def create_price_monitoring_run(
    ...
    write_price_fetcher_input_csv(input_csv_path, selection_result.items)
...
def write_price_fetcher_input_csv(path: Path, products: list[SelectedPriceMonitoringProduct]) -> Path:
...
def _latest_fetch_payload(run_dir: Path) -> dict | None:
    ...
    if execution_payload:
        return {
            ...
            "legacy_marketplace_fetch_used": bool(execution_payload.get("legacy_marketplace_fetch_used", False)),
