...
from .publish_execution import (
    OPENCART_IMPORT_REPORT_NAME,
    OPENCART_UPLOAD_REPORT_NAME,
    ...
    *,
    ...
    return {
        "publish_attempted": bool(
            publish_result.details.get("publish_attempted", True)
        ),
        ...
        "upload_report_path": str(
            publish_result.details.get("upload_report_path") or upload_report_path
        ),
        "import_report_path": str(
            publish_result.details.get("import_report_path") or import_report_path
        ),
        "publish_metadata_path": (
            str(publish_result.artifacts.metadata_path)
            if publish_result.artifacts.metadata_path
            else None
        ),
    }
...
def publish_product(request: PublishRequest) -> ServiceResult:
    ...
    publish_message = (
        str(result["publish_message"]) if result.get("publish_message") else None
    )
    warnings: list[str] = []
    ...
    return ServiceResult(
        ...
        artifacts=RunArtifacts(
            model_root=(
                Path(result["metadata_path"]).parent
                if result.get("metadata_path")
                else None
            ),
            published_csv_path=(
                Path(result["published_csv_path"])
                if result.get("published_csv_path")
                else None
            ),
            metadata_path=(
                Path(result["metadata_path"]) if result.get("metadata_path") else None
            ),
        ),
        details={
            ...
            "upload_report_path": (
                str(result["upload_report_path"])
                if result.get("upload_report_path")
                else None
            ),
            "import_report_path": (
                str(result["import_report_path"])
                if result.get("import_report_path")
                else None
            ),
        },
    )
