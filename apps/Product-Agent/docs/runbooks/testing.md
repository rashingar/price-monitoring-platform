...
Run tests from `scraper/` and always use the repo virtual environment. Do not use bare `python`, `py`, or a global interpreter.
...
Regenerate the OpenAPI snapshot from `scraper/` after intentional backend API changes:
...
All commands in this runbook must use `..\.venv\Scripts\python.exe` from `scraper/`; do not use bare `python`, `py`, or a global interpreter.
