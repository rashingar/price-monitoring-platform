...
from ecommerce.db.models.vendor_sources import (
    Vendor,
...
def test_domain_model_imports_share_one_metadata_registry() -> None:
    ...
    assert {model.__table__.metadata for model in representative_models} == {
        BaseFromBase.metadata
    }
...
def test_metadata_loader_path_supports_alembic_table_discovery() -> None:
    ...
    assert (
        sorted(table.name for table in BaseFromBase.metadata.sorted_tables)
        == EXPECTED_TABLE_NAMES
    )
...
def test_model_package_does_not_reexport_model_classes_or_base() -> None:
    ...
    assert [
        name for name in sorted(removed_exports) if hasattr(model_package, name)
    ] == []
