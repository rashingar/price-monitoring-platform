...
def _add_source(
    session, *, model: str, vendor: str = "skroutz", status: str = "active"
) -> ProductSource:
    ...
    def fake_capture(url: str, *, vendor_slug: str | None = None) -> CaptureResult:
        ...
        return CaptureResult(
            ...
            snapshot=CaptureSnapshotPayload(
                ...
                error_message=(
                    "blocked"
                    if reason_code == "FIRECRAWL_API_FAILED"
                    else "parse failed"
                ),
                captured_at=NOW,
                fetched_at=NOW,
            ...
            error_code=reason_code,
            error_message=(
                "blocked" if reason_code == "FIRECRAWL_API_FAILED" else "parse failed"
            ),
        )
...
def test_firecrawl_threshold_config(monkeypatch) -> None:
    monkeypatch.delenv(
        "ECOMMERCE_FIRECRAWL_SOURCE_REVIEW_FAILURE_THRESHOLD", raising=False
    )
...
def test_source_health_maps_firecrawl_failure_reason(tmp_path: Path) -> None:
    ...
    with session_scope(database_url) as session:
        ...
        capture_due_product_sources(
            session, capture_fn=_firecrawl_failure("FIRECRAWL_PARSE_FAILED")
        )
    ...
    assert (
        firecrawl_health_reason(
            capture_strategy="skroutz_firecrawl", error_code="FIRECRAWL_TIMEOUT"
        )
        == "firecrawl_timeout"
    )


def test_repeated_firecrawl_blocked_moves_skroutz_source_url_to_needs_review(
    ...
    with session_scope(database_url) as session:
        ...
        capture_due_product_sources(
            session, capture_fn=_firecrawl_failure("FIRECRAWL_API_FAILED")
        )
...
def test_repeated_firecrawl_parse_failed_moves_skroutz_source_url_to_needs_review(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.setenv("ECOMMERCE_FIRECRAWL_SOURCE_REVIEW_FAILURE_THRESHOLD", "2")
    database_url = _database_url(tmp_path)
...
def test_threshold_one_escalates_after_one_matching_failure(
    tmp_path: Path, monkeypatch
) -> None:
...
def test_one_off_firecrawl_failure_does_not_escalate_when_threshold_two(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.setenv("ECOMMERCE_FIRECRAWL_SOURCE_REVIEW_FAILURE_THRESHOLD", "2")
    database_url = _database_url(tmp_path)
...
def test_bestprice_is_not_escalated_by_firecrawl_like_flags(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.setenv("ECOMMERCE_FIRECRAWL_SOURCE_REVIEW_FAILURE_THRESHOLD", "1")
    database_url = _database_url(tmp_path)
    with session_scope(database_url) as session:
        ...
        capture_due_product_sources(
            session, vendor_slug="bestprice", capture_fn=_bestprice_failure
        )
        source_url = session.query(SourceUrl).one()
...
def test_manual_recapture_can_trigger_firecrawl_escalation(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.setenv("ECOMMERCE_FIRECRAWL_SOURCE_REVIEW_FAILURE_THRESHOLD", "1")
    database_url = _database_url(tmp_path)
...
def test_manual_recapture_route_captures_exactly_one_and_forces_not_due(
    tmp_path: Path, monkeypatch
) -> None:
    database_url = _database_url(tmp_path)
    ...
    monkeypatch.setattr(
        "ecommerce.vendor_sources.capture_service.capture_source_url",
        successful_capture,
    )
    response = TestClient(create_app()).post(
        f"/api/vendor-sources/source-health/{first_id}/recapture"
    )
...
def test_manual_recapture_route_rejects_missing_and_inactive_sources(
    tmp_path: Path, monkeypatch
) -> None:
    database_url = _database_url(tmp_path)
    monkeypatch.setenv(DATABASE_URL_ENV_VAR, database_url)
    ...
    assert (
        client.post("/api/vendor-sources/source-health/999999/recapture").status_code
        == 404
    )
    assert (
        client.post(
            f"/api/vendor-sources/source-health/{inactive_id}/recapture"
        ).status_code
        == 404
    )
