...
def _enqueue_job(
    ...
    store = _job_store(api_request)
    if job_type == JobType.FULL_PIPELINE:
        try:
            record = store.enqueue_full_pipeline_unless_active(payload)
        ...
        record = store.enqueue(job_type, payload)
...
def authoring_intro_job(
    request: AuthoringIntroJobRequest, api_request: Request
) -> JobResponse:
...
def authoring_seo_job(
    request: AuthoringSeoJobRequest, api_request: Request
) -> JobResponse:
...
def full_pipeline_job(
    request: FullPipelineJobRequest, api_request: Request
) -> JobResponse:
    return _enqueue_job(api_request, JobType.FULL_PIPELINE, _request_payload(request))


@router.post(
    "/{job_id}/stop", response_model=JobResponse, responses=_NOT_FOUND_RESPONSE
)
def stop_job(
    job_id: str, api_request: Request, request: StopJobRequest | None = None
) -> JobResponse:
    try:
...
@router.post(
    "/{job_id}/retry", response_model=JobResponse, responses=_NOT_FOUND_RESPONSE
)
def retry_job(job_id: str, api_request: Request) -> JobResponse:
    ...
    if not is_terminal_job_status(record.status):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Only terminal jobs can be retried.",
        )
...
@router.post(
    "/{job_id}/start", response_model=JobResponse, responses=_NOT_FOUND_RESPONSE
)
...
def list_jobs(api_request: Request) -> JobListResponse:
    return JobListResponse(
        jobs=[
            ...
            for record in _job_store(api_request).list_jobs()
        ]
    )
...
@router.get(
    "/{job_id}/logs", response_model=JobLogsResponse, responses=_NOT_FOUND_RESPONSE
)
def get_job_logs(job_id: str, api_request: Request) -> JobLogsResponse:
    _get_job_response(api_request, job_id)
    return JobLogsResponse(
        job_id=job_id, lines=_job_store(api_request).read_logs(job_id)
    )
