...
def _service_error_from_prepare_metadata_failure(
...
def _degraded_prepare_result_from_metadata_error(
    ...
    return _build_prepare_service_result(
        ...
        error_code=(
            ...
            if payload.run.status == RunStatus.COMPLETED
            else payload.run.error_code
        ),
        error_detail=(
            str(error)
            if payload.run.status == RunStatus.COMPLETED
            else payload.run.error_detail
        ),
        artifacts=RunArtifacts(
            ...
            source_report_json_path=_existing_path(
                payload.artifacts.source_report_json_path
            ),
            llm_task_manifest_path=_existing_path(
                payload.artifacts.llm_task_manifest_path
            ),
            intro_text_context_path=_existing_path(
                payload.artifacts.intro_text_context_path
            ),
            intro_text_prompt_path=_existing_path(
                payload.artifacts.intro_text_prompt_path
            ),
            intro_text_output_path=payload.artifacts.intro_text_output_path,
            seo_meta_context_path=_existing_path(
                payload.artifacts.seo_meta_context_path
            ),
        ...
        details={
            ...
            "matched_schema_id": str(
                payload.details.get("matched_schema_id", "") or ""
            ),
            "schema_score": float(payload.details.get("schema_score", 0.0) or 0.0),
            "warnings_count": int(
                payload.details.get("warnings_count", len(payload.run.warnings)) or 0
            ),
            "llm_prepare_mode": str(
                payload.details.get("llm_prepare_mode", "split_tasks") or "split_tasks"
            ),
        },
    )
...
def prepare_product(request: PrepareRequest) -> ServiceResult:
    ...
    gallery_settings = (
        gallery_settings_payload if isinstance(gallery_settings_payload, dict) else {}
    )
    characteristics_settings_payload = report.get("characteristics_settings", {})
    characteristics_settings = (
        characteristics_settings_payload
        if isinstance(characteristics_settings_payload, dict)
        else {}
    )
    details = {
        ...
        "product_code": str(
            getattr(getattr(parsed, "source", None), "product_code", "") or ""
        ),
        ...
        "llm_prepare_mode": (
            "blocked_snapshot"
            if str(getattr(getattr(parsed, "source", None), "page_type", "") or "")
        ...
        details.update(
            {
                "gallery_url_used": bool(
                    gallery_settings.get("gallery_url_used", False)
                ),
                "gallery_extraction_url": str(
                    gallery_settings.get("gallery_extraction_url", "") or ""
                ),
                "second_opencart_image_index": gallery_settings.get(
                    "second_opencart_image_index"
                ),
                "second_opencart_image_override_applied": bool(
                    gallery_settings.get(
                        "second_opencart_image_override_applied", False
                    )
                ),
                "gallery_mode": str(gallery_settings.get("gallery_mode", "") or ""),
                "gallery_whole_mode": bool(
                    gallery_settings.get("gallery_whole_mode", False)
                ),
                "gallery_source_filter_domain": str(
                    gallery_settings.get("source_filter_domain", "") or ""
                ),
                "gallery_extracted_before_source_filter_count": gallery_settings.get(
                    "extracted_before_source_filter_count"
                ),
                "gallery_after_source_filter_count": gallery_settings.get(
        ...
        details.update(
            {
                "characteristics_url_used": bool(
                    characteristics_settings.get("characteristics_url_used", False)
                ),
                "characteristics_extraction_url": str(
                    characteristics_settings.get("characteristics_extraction_url", "")
                    or ""
                ),
            }
        )
