...
def score_candidate(
    *,
    ...
    extra_snippets = " ".join(
        str(value) for value in getattr(item, "extra_snippets", ()) or ()
    )
...
def _identity_score(product: SourceResolutionProduct, haystack: str) -> float:
    ...
    for value in (
        product.mpn,
        product.barcode,
        product.metadata.get("mpn", ""),
        product.metadata.get("barcode", ""),
    ):
...
def _name_identifier_score(name: str, haystack: str) -> float:
    ...
    for chunk in chunks:
        if (
            _has_alpha_and_digit(chunk)
            and len(chunk) >= 4
            and chunk in normalized_haystack
        ):
            score += 10.0
            matched_chunks += 1
            continue
...
def _has_alpha_and_digit(value: str) -> bool:
    return any(char.isalpha() for char in value) and any(
...
def _manufacturer_score(product: SourceResolutionProduct, haystack: str) -> float:
    manufacturer = str(
        product.brand or product.metadata.get("manufacturer") or ""
    ).strip()
