...
class SourceResolutionConfig:
    ...
    def with_preferred_sources(self, source_names: tuple[str, ...]) -> "SourceResolutionConfig":
        allowed = {name.casefold() for name in source_names}
        sources = tuple(source for source in self.preferred_sources if source.source_name in allowed)
        missing = allowed - {source.source_name for source in sources}
        if missing:
            raise SourceResolutionConfigError(f"configured preferred_sources missing required sources: {', '.join(sorted(missing))}")
