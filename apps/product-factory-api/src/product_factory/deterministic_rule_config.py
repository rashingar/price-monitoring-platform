...
def _normalize_differentiator_specs(
    raw_specs: Any,
...
def _parse_generic_output_profile(payload: dict[str, Any]) -> GenericOutputProfile:
    ...
    return GenericOutputProfile(
        ...
        meta_title=normalize_for_match(values.get("meta_title"))
        or DEFAULT_GENERIC_OUTPUTS["meta_title"],
        seo_keyword=normalize_for_match(values.get("seo_keyword"))
        or DEFAULT_GENERIC_OUTPUTS["seo_keyword"],
    )
...
def _parse_generic_rule(
    ...
    *,
    ...
    source_scope = _normalize_source_scopes(
        payload.get("source_scope"), default_scope=default_scope
    )[0]
    ...
    if not match_leaf_category and not match_sub_category:
        raise ValueError(
            "generic deterministic rule requires match.leaf_category or match.sub_category"
        )
    ...
    differentiator_specs = _normalize_differentiator_specs(
        payload.get("differentiator_specs", [])
    )
    max_differentiators = int(
        payload.get("max_differentiators")
        or len(differentiator_specs)
        or default_max_differentiators
    )
...
def _parse_source_rule(
    ...
    match = payload.get("match") if isinstance(payload.get("match"), dict) else {}
    source_scopes = _normalize_source_scopes(
        payload.get("source_scope"), default_scope=default_scope
    )
    ...
    for key, raw_value in raw_outputs.items():
        ...
        if not values:
            raise ValueError(
                f"source deterministic rule output {key!r} must contain values"
            )
...
def load_deterministic_rule_config(
    path: str = str(NAME_RULES_PATH),
) -> DeterministicRuleConfig:
    payload = _load_json(Path(path))
    rule_defaults = (
        payload.get("rule_defaults")
        if isinstance(payload.get("rule_defaults"), dict)
        else {}
    )
    default_scope = normalize_for_match(rule_defaults.get("source_scope")) or "any"
    if default_scope not in VALID_SOURCE_SCOPES:
        raise ValueError(
            f"Unsupported deterministic rule default source_scope: {rule_defaults.get('source_scope')!r}"
        )
    generic_outputs = _parse_generic_output_profile(
        payload.get("generic_outputs")
        if isinstance(payload.get("generic_outputs"), dict)
        else {}
    )

    generic_rules = tuple(
        _parse_generic_rule(
            rule, default_scope=default_scope, default_max_differentiators=3
        )
...
def _matches_generic_rule(
    rule: GenericNameRule, *, source_name: str, sub_category: str, leaf_category: str
) -> tuple[bool, bool]:
    ...
    targets = [
        target
        for target in (
            normalize_for_match(rule.match_sub_category),
            normalize_for_match(rule.match_leaf_category),
        )
        if target
    ]
    candidates = [
        candidate for candidate in (normalized_sub, normalized_leaf) if candidate
    ]
    ...
    *,
    ...
    for rule in config.generic_rules:
        ...
        if matches and exact:
            return ResolvedGenericNameRule(
                rule=rule, matched_exact=True, outputs=config.generic_outputs
            )
    ...
    for rule in config.generic_rules:
        ...
        if matches and not exact:
            return ResolvedGenericNameRule(
                rule=rule, matched_exact=False, outputs=config.generic_outputs
            )
    return None
...
def resolve_source_scoped_rule(
    *,
    ...
    for rule in config.source_rules:
        if (
            "any" not in rule.source_scopes
            and normalized_source not in rule.source_scopes
        ):
            continue
        if normalized_family != rule.match_family:
            continue
        if (
            rule.match_leaf_category
            and normalize_for_match(rule.match_leaf_category) != normalized_leaf
        ):
            continue
        if (
            rule.match_sub_category
            and normalize_for_match(rule.match_sub_category) != normalized_sub
        ):
            continue
        return rule
    return None
