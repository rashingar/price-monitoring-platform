...
from pricefetcher.db.product_source_repository import create_product_from_source_urls
from pricefetcher.db.repositories import product_to_dict
...
def post_product_from_source(request: ProductFromSourceRequest) -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Product source ingestion failed: {_safe_db_error(exc)}") from exc
