"""Start the PriceFetcher backend API for local development."""
...
from pricefetcher.db.diagnostics import collect_database_status
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Start the PriceFetcher FastAPI backend.")
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if _print_existing_pricefetcher_api_status(args.host, args.port):
        return

    uvicorn.run("pricefetcher.api.app:app", host=args.host, port=args.port, reload=args.reload)


def _print_existing_pricefetcher_api_status(host: str, port: int) -> bool:
    if _can_bind(host, port):
        return False

    health_url = f"http://{host}:{port}/api/health"
    health = _read_json_url(health_url)
    if health.get("status") == "ok" and health.get("service") == "price-fetcher":
        print(f"PriceFetcher API is already running on {host}:{port}.")
        print(f"Health URL: {health_url}")
        return True

    raise SystemExit(
        f"Cannot start PriceFetcher API because {host}:{port} is already in use by another process. "
...
def _print_db_setup_hints() -> None:
    if not is_database_configured():
        ...
        print("Set PRICEFETCHER_DATABASE_URL and run alembic upgrade head to enable Price Monitoring DB persistence.")
        return
