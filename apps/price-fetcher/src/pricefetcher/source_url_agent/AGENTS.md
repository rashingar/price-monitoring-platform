...
src/pricefetcher/source_url_agent/
...
## Relationship to the Rest of `price-fetcher`

This package should reuse existing repository functionality:

* `pricefetcher.source_urls`
* `pricefetcher.db.source_url_repository`
* `pricefetcher.db.models`
* `pricefetcher.source_url_import`
* `pricefetcher.source_url_agent.search`
* `pricefetcher.price_monitoring.source_url_coverage`
...
Normalize candidate URLs using existing `pricefetcher.source_urls.normalize_source_url`.
...
python -m pricefetcher.jobs.source_url_agent ...


Suggested commands:

powershell
python -m pricefetcher.jobs.source_url_agent run `
...
powershell
python -m pricefetcher.jobs.source_url_agent from-catalog `
  --source all `
  --missing-only `
  --limit 20 `
  --dry-run
...
powershell
python -m pricefetcher.jobs.source_url_agent apply-review `
  --review-file output/source_url_agent/runs/{run_id}/needs_review_source_urls_reviewed.csv `
  --dry-run


powershell
python -m pricefetcher.jobs.source_url_agent analyze `
  --run-id {run_id}
...
Electronet reference logic may exist in `../Product-Agent`.
