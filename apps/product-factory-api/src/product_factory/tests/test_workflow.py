...
from product_factory.models import CLIInput, GalleryImage, ParsedProduct, SchemaMatchResult, SourceProductData, SpecItem, SpecSection, TaxonomyResolution
from product_factory.services.execution_models import PrepareExecutionResult, RenderExecutionResult, RenderExecutionValidationReport
from product_factory.services import PrepareRequest, PublishRequest, RenderRequest, RunArtifacts, RunMetadata, RunStatus, RunType, ServiceError, ServiceErrorCode, ServiceResult
from product_factory.workflow import build_cli_input_from_args, build_parser, prepare_workflow, render_workflow, resolve_model_for_render
...
def test_prepare_workflow_writes_prompt_artifacts(tmp_path: Path, monkeypatch) -> None:
    ...
    assert "Use only `<strong>` and `</strong>` for inline emphasis." in intro_text_prompt
...
def test_resolve_render_sections_preserves_source_order_when_backfilling_weak_sections() -> None:
    from product_factory.services import render_execution

    sections, warnings = render_execution._resolve_render_sections(
        extracted_sections=[
            {"title": "Usable One", "body_text": " ".join(f"λέξη{i}" for i in range(30)), "image_url": "https://example.com/1.jpg"},
            {"title": "Weak Two", "body_text": " ".join(f"όρος{i}" for i in range(12)), "image_url": "https://example.com/2.jpg"},
            {"title": "Usable Three", "body_text": " ".join(f"κείμενο{i}" for i in range(30)), "image_url": ""},
        ],
        sections_requested=3,
    )

    assert [section["title"] for section in sections] == ["Usable One", "Weak Two", "Usable Three"]
