...
from pipeline.models import CLIInput, ParsedProduct, SourceProductData, TaxonomyResolution
from pipeline.services.authoring_service import (
    PreparedAuthoringArtifactsNotFoundError,
    get_authoring_status,
    run_intro_text_authoring,
    run_seo_meta_authoring,
)
from pipeline.services.llm_stage_execution import SplitLLMStageResult, SplitLLMTaskPaths
from pipeline.services.render_execution import _build_llm_validation_backstop_errors
from pipeline.services.settings_service import (
    IntroTextPolicy,
...
def test_authoring_api_routes_are_included_and_map_errors(isolated_repo: Path, monkeypatch) -> None:
    fastapi_testclient = pytest.importorskip("fastapi.testclient")
    import pipeline.api.routes_authoring as routes_authoring
    from pipeline.api.app import create_app
    from pipeline.services.errors import ServiceError, ServiceErrorCode
